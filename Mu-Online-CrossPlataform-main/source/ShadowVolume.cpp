...
void RenderShadowToScreen( void)
{
	...
	platform::GetRenderBackend().DrawTriangleFan2D(quad, 4, false);
	platform::GetRenderBackend().SetDepthFunction(GL_LESS);
	glDisable( GL_STENCIL_TEST);
	EnableDepthMask();
}
...
{
	std::vector<platform::Vertex3D> vertices(static_cast<size_t>(m_nNumVertices));
	for ( int i = 0; i < m_nNumVertices; ++i)
	{
		vertices[i].x = m_pVertices[i][0];
		vertices[i].y = m_pVertices[i][1];
		vertices[i].z = m_pVertices[i][2];
		vertices[i].u = 0.f;
		vertices[i].v = 0.f;
		vertices[i].r = 0.f;
		vertices[i].g = 0.f;
	...
	platform::GetRenderBackend().DrawTriangleList3D(vertices.data(), m_nNumVertices, false, false);
}
