...
CUISocketListBox::CUISocketListBox()
{
	...
	SetPosition(2* GetCenterX(640) + 275, 360);
	SetSize(160, 65);
}
...
void CUIRenderTextOriginal::UploadText(int sx,int sy,int Width,int Height)
{
	...
	if(Width > 0 && Height > 0 && sx+Width > 0 && sy+Height > 0)
	{
		...
		platform::LegacyCompatibleTexImage2D(GL_TEXTURE_2D, 0, b->Components, (int)b->Width, (int)b->Height, 0, GL_RGBA, GL_UNSIGNED_BYTE, b->Buffer);
...
CUIBCDeclareGuildListBox::CUIBCDeclareGuildListBox()
{
	...
	SetPosition(2 * GetCenterX(640) + 465, 364);
	SetSize(160, 235);
}
...
CUIInGameShopListBox::CUIInGameShopListBox()
{
	...
	SetPosition(setPosMidRight(490), 360);
}
