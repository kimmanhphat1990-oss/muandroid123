...
bool OpenJpegBuffer(char *filename,float *BufferFloat)
{
	...
	char szRelativePath[_MAX_PATH] = { 0 };
	strcpy(szRelativePath, "Data\\");
	strcat(szRelativePath, NewFileName);
	strcat(szRelativePath, "OZJ");

	if (false == platform::CopyResolvedGameAssetPath(FileName, _countof(FileName), szRelativePath))
	{
		char Text[256];
		sprintf(Text, "%s - Path resolve failed.", szRelativePath);
...
#endif // KJH_ADD_INGAMESHOP_UI_SYSTEM
{
	...
	{
		if (false == platform::CopyResolvedGameAssetPath(szFullPath, _countof(szFullPath), szFileName))
		{
			char szErrorMsg[256] = {0, };
			sprintf(szErrorMsg, "LoadBitmap Path Failed: %s", szFileName);
