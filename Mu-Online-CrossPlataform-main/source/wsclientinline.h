...
#if defined(__ANDROID__)
#define MU_ANDROID_SYSTEM_CHAT(message_index) do { (void)(message_index); } while (0)
#else
#define MU_ANDROID_SYSTEM_CHAT(message_index) \
	do \
	{ \
		g_pChatListBox->AddText("", GlobalText[(message_index)], SEASON3B::TYPE_SYSTEM_MESSAGE); \
	} while (0)
#endif
...
#define SendRequestWareHouseOpen() \
{ \
	CStreamPacketEngine spe; \
	spe.Init(0xC1, 0xF3); \
	spe << (BYTE)0x53; \
...
{\
	...
	spe.Send();\
	MU_ANDROID_SYSTEM_CHAT(470);\
	MU_ANDROID_SYSTEM_CHAT(471);\
}
...
{\
	...
	spe << Serial[i];\
	spe.Send( TRUE);\
	MU_ANDROID_SYSTEM_CHAT(472);\
	MU_ANDROID_SYSTEM_CHAT(473);\
}

#define SendRequestCreateAccount( p_lpszID, p_lpszPassword, p_lpszPCode, p_lpszEmail)\
{\
	strcpy(LogInID, ( p_lpszID));\
	CStreamPacketEngine spe;\
	spe.Init( 0xC1, 0xF1);\
	...
	ZeroMemory( lpszID, MAX_ID_SIZE+1);\
	ZeroMemory( lpszPass, MAX_PASSWORD_SIZE+1);\
	ZeroMemory( lpszPCode, 20);\
	ZeroMemory( lpszMail, 50);\
	strncpy(lpszID, p_lpszID, MAX_ID_SIZE);\
	strncpy(lpszPass, p_lpszPassword, MAX_PASSWORD_SIZE);\
	strncpy(lpszPCode, p_lpszPCode, 20);\
	strncpy(lpszMail, p_lpszEmail, 50);\
	BuxConvert(( BYTE*)lpszID,MAX_ID_SIZE);\
	BuxConvert(( BYTE*)lpszPass,MAX_PASSWORD_SIZE);\
	BuxConvert(( BYTE*)lpszPCode,20);\
	BuxConvert(( BYTE*)lpszMail,50);\
	spe.AddData( lpszID, MAX_ID_SIZE);\
	spe.AddData( lpszPass, MAX_PASSWORD_SIZE);\
	spe.AddData( lpszPCode, 20);\
	spe.AddData( lpszMail, 50);\
	spe << GetTickCount();\
	for(int i=0;i<SIZE_PROTOCOLVERSION;i++)\
	spe << ( BYTE)( Version[i]-(i+1));\
	for(int i=0;i<SIZE_PROTOCOLSERIAL;i++)\
...
__forceinline void SendChat(const char* Text)
{
	...
	{
		BOOL bReturn = TRUE;
		const char *lspzCommand[2] = { "/", "!" };
		for ( int i = 0; i < sizeof ( lspzCommand) / sizeof ( const char*); i++)
		{
	...
	spe.AddData((void*)Text, (WORD)std::min<size_t>(strlen(Text) + 1, MAX_CHAT_SIZE));
	spe.Send();
}
...
#define SendRequestMagic( p_Type, p_Key)\
{\
	if(!FindText2(Hero->ID,"webzen") && ( p_Type==40 || abs( (int)(GetTickCount() - g_dwLatestMagicTick)) > 0 ))\
	{\
		g_dwLatestMagicTick = GetTickCount();\
		CStreamPacketEngine spe;\
		WORD Type = (WORD)p_Type;\
...
#define SendRequestTrade( p_Key, istradex)\
{\
...
#define SendRequestTradeXResult( p_Result, tradeX, Cancel)\
{\
	CStreamPacketEngine spe;\
	spe.Init( 0xC1, 0x3C);\
	spe << ( BYTE)( p_Result);\
...
__forceinline void SendRequestChangeWare(int targetWarehouse)
{
	int Flag = 0;
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x84);
	spe << (BYTE)Flag << (DWORD)targetWarehouse;
	spe.Send();

	g_ConsoleDebug->Write(MCD_SEND, "0x84 Send [SendRequestChangeWare(%d)]", targetWarehouse);
}
...
#define SendChaosGenesisClose( iType, iLevel )\
{	\
	CStreamPacketEngine spe;\
	spe.Init( 0xC1, 0xEF);\
	spe << (BYTE)0x23;\
...
#define SendRequestStartHelper(status)\
{\
	CStreamPacketEngine spe;\
	spe.Init( 0xC1, 0xBF);\
	spe << (BYTE)0x51;\
...
#define SendRequestSaveHelperSetting(buffer)\
{\
	CStreamPacketEngine spe;\
	spe.Init( 0xC2, 0xAE);\
	spe << (BYTE)0;\
	spe.AddData(buffer, sizeof(buffer)); \
	spe.Send();\
}
#endif
