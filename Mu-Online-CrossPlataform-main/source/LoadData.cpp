...
void CLoadData::AccessModel(int Type,const char *Dir,const char *FileName,int i)
{
	...
	Success = Models[Type].Open2(const_cast<char*>(Dir), Name);
...
void CLoadData::OpenTexture(int Model,const char *SubFolder, int Wrap, int Type,bool Check)
{
	...
	for(int i=0;i<pModel->NumMeshs;i++)
	{
		...
		strcat(szRelativePath, SubFolder);
		strcat(szRelativePath, pTexture->FileName);

		if (false == platform::CopyResolvedGameAssetPath(szFullPath, _countof(szFullPath), szRelativePath))
		{
			char szErrorMsg[_MAX_PATH] = { 0 };
			sprintf(szErrorMsg, "OpenTexture Path Failed: %s of %s", szRelativePath, pModel->Name);
