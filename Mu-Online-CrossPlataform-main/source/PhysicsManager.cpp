...
namespace
{
	float GetClothTexCoordU(int xVertex, int numHor)
	{
		return ( float)xVertex / ( float)( numHor - 1);
	}

	float GetClothTexCoordV(int yVertex, int numVer)
	{
		return min( 0.99f, ( float)yVertex / ( float)( numVer - 1));
	}

	void SetClothVertex(platform::Vertex3D& vertex, vec3_t *pvRenderPos, int numHor, int numVer, int xVertex, int yVertex, float r, float g, float b, float a)
	{
		int iVertex = numHor * yVertex + xVertex;
		vec3_t *pvPos = &pvRenderPos[iVertex];
		vertex.x = ( *pvPos)[0];
		vertex.y = ( *pvPos)[1];
		vertex.z = ( *pvPos)[2];
		vertex.u = GetClothTexCoordU(xVertex, numHor);
		vertex.v = GetClothTexCoordV(yVertex, numVer);
		vertex.r = r;
		vertex.g = g;
		vertex.b = b;
		vertex.a = a;
	...
	void AppendClothTriangle(std::vector<platform::Vertex3D>& vertices, vec3_t *pvRenderPos, int numHor, int numVer,
		int x0, int y0, int x1, int y1, int x2, int y2, float r, float g, float b, float a)
	{
		platform::Vertex3D vertex;
		SetClothVertex(vertex, pvRenderPos, numHor, numVer, x0, y0, r, g, b, a);
		vertices.push_back(vertex);
		SetClothVertex(vertex, pvRenderPos, numHor, numVer, x1, y1, r, g, b, a);
		vertices.push_back(vertex);
		SetClothVertex(vertex, pvRenderPos, numHor, numVer, x2, y2, r, g, b, a);
		vertices.push_back(vertex);
	}

	void AppendClothQuad(std::vector<platform::Vertex3D>& vertices, vec3_t *pvRenderPos, int numHor, int numVer,
		int x0, int y0, int x1, int y1, int x2, int y2, int x3, int y3, float r, float g, float b, float a)
	{
		AppendClothTriangle(vertices, pvRenderPos, numHor, numVer, x0, y0, x1, y1, x2, y2, r, g, b, a);
		AppendClothTriangle(vertices, pvRenderPos, numHor, numVer, x0, y0, x2, y2, x3, y3, r, g, b, a);
	}
}
	...
	if ( PCT_MASK_LIGHT&m_dwType )
    {
...
#ifdef RENDER_CLOTH
     {
		 ...
		 if (ret)
		 {
			 ...
			 std::vector<platform::Vertex3D> shine_vertices;
			 shine_vertices.reserve((m_iNumHor - 1) * (m_iNumVer - 1) * 6);

			 for (int j = 0; j < m_iNumVer - 1; ++j)
			 {
				 for (int i = 0; i < m_iNumHor - 1; ++i)
				 {
					 AppendClothQuad(shine_vertices, pvRenderPos, m_iNumHor, m_iNumVer,
						 i, j,
						 i + 1, j,
						 i + 1, j + 1,
						 i, j + 1,
						 lights, lights, lights, 1.0f);
				 }
			 }
			 if (!shine_vertices.empty())
			 {
				 platform::GetRenderBackend().DrawTriangleList3D(&shine_vertices[0], static_cast<int>(shine_vertices.size()), true, true);
			 }
		 }
     }
...
void CPhysicsCloth::RenderFace( BOOL bFront, int iTexture, vec3_t *pvRenderPos)
{
	...
	std::vector<platform::Vertex3D> face_vertices;
	face_vertices.reserve((m_iNumHor - 1) * (m_iNumVer - 1) * 6);
	...
	if ( bFront)
	{
		for ( int j = 0; j < m_iNumVer - 1; ++j)
		{
			for ( int i = 0; i < m_iNumHor - 1; ++i)
			{
				AppendClothQuad(face_vertices, pvRenderPos, m_iNumHor, m_iNumVer,
					i, j,
					i + 1, j,
					i + 1, j + 1,
					i, j + 1,
	...
	{
		platform::GetRenderBackend().DrawTriangleList3D(&face_vertices[0], static_cast<int>(face_vertices.size()), true, false);
	}
}
...
void CPhysicsCloth::RenderCollisions( void)
{
	...
	for ( ; pHead; pHead = m_lstCollision.GetNext( pHead))
	{
		...
		if ( CLT_SPHERE == pCol->GetType())
		{
			...
			platform::GetRenderBackend().TranslateModelView(vCenter[0], vCenter[1], vCenter[2]);
...
void CPhysicsManager::Move( float fTime)
{
	...
	CPhysicsManager::s_fWind += fPlus * FPS_ANIMATION_FACTOR;
