...
class CBTree
{
	protected:
		...
		void CycleFrom( CBNode<T, S>* pNode, uintptr_t dwParam);
		void ( *m_Process)( T Data, S CompValue, uintptr_t dwParam);
	...
	public:
		...
		void Cycle( void ( *Process)( T Data, S CompValue, uintptr_t dwParam), uintptr_t dwParam);
...
template <class T, class S>
void CBTree<T, S>::Cycle( void ( *Process)( T Data, S CompValue, uintptr_t dwParam), uintptr_t dwParam)
{
	m_Process = Process;
...
template <class T, class S>
void CBTree<T, S>::CycleFrom( CBNode<T, S>* pNode, uintptr_t dwParam)
{
	if ( pNode->GetLeft() != NULL)
	{
...
template <class T, class S>
void ProcessGetOptimizeList( T Data, S CompValue, uintptr_t dwParam)
{
	uintptr_t *pdwParam = reinterpret_cast<uintptr_t*>(dwParam);
	long *pCount = reinterpret_cast<long*>(pdwParam[0]);
	T *pData = reinterpret_cast<T*>(pdwParam[1]);
	S *pCompValue = reinterpret_cast<S*>(pdwParam[2]);
	pData[*pCount] = Data;
...
BOOL CBTree<T, S>::GetOptimizeList( T *pData, S *pCompValue)
{
	...
	uintptr_t dwOptimizeData[3] =
	{
		reinterpret_cast<uintptr_t>(&lOptimizeCount),
		reinterpret_cast<uintptr_t>(pData),
		reinterpret_cast<uintptr_t>(pCompValue)
	};
	Cycle(ProcessGetOptimizeList, reinterpret_cast<uintptr_t>(dwOptimizeData));
