...
bool CPacketManager::LoadEncryptionKey(const char* name) // OK
{
	return this->LoadKey(name,4370,0);
}

bool CPacketManager::LoadDecryptionKey(const char* name) // OK
{
	return this->LoadKey(name,4370,1);
}

bool CPacketManager::LoadKey(const char* name,WORD header,bool type) // OK
{
	...
	size_t size = fread(&HeaderInfo, 1, sizeof(HeaderInfo), file);
	if (size != sizeof(HeaderInfo))
	{
		fclose(file);
		return 0;
	}
	...
	size = fread(table, 1, sizeof(table), file);
