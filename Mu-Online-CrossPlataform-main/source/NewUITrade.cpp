...
bool CNewUITrade::Create(CNewUIManager* pNewUIMng, int x, int y)
{
	...
	m_pYourInvenCtrl->LockInventory();	// »ó´ë¹æ ¹°Ç°Àº Å¬¸¯ ¾ÈµÇ°Ô ÇÔ.
...
const POINT& SEASON3B::CNewUITrade::GetPos() const
{
	return m_Pos;
}
...
bool CNewUITrade::UpdateMouseEvent()
{
	...
	{
		gTradeX.UpdateMouseEvent();
		return 1;
	}
	else
	{
		if ((m_pYourInvenCtrl && false == m_pYourInvenCtrl->UpdateMouseEvent()) || (m_pMyInvenCtrl && false == m_pMyInvenCtrl->UpdateMouseEvent()))
		{
			if (SEASON3B::IsPress(VK_LBUTTON) && CNewUIInventoryCtrl::GetPickedItem()->GetOwnerInventory() == m_pMyInvenCtrl && m_bMyConfirm)
			{
				m_bMyConfirm = false;
...
void CNewUITrade::RenderBackImage()
{
	...
	RenderImage(IMAGE_TRADE_MONEY, m_Pos.x + 52, m_Pos.y + 150, 170.f, 26.f);
	RenderImage(IMAGE_TRADE_MONEY, m_Pos.x + 52, m_Pos.y + 356, 170.f, 26.f);

	float fSrcY = m_bYourConfirm ? 24.f : 0.f;
	RenderImage(IMAGE_TRADE_CONFIRM, m_Pos.x + 146, m_Pos.y + 186, 24, 24, 0.0f, fSrcY, 0);
	
	if (CheckMouseIn(m_Pos.x + 146, m_Pos.y + 390, 24, 24))
	{
		RenderTipText(m_Pos.x + 149, m_Pos.y + 378, GlobalText[228]);
	}

	float sSrcY = m_bMyConfirm ? 24.f : 0.f;
	RenderImage(IMAGE_TRADE_CONFIRM, m_Pos.x + 146, m_Pos.y + 390, 24, 24, 0.0f, sSrcY, 0);
}
...
void CNewUITrade::RenderText()
{
	...
	g_pRenderText->RenderText(m_Pos.x, m_Pos.y + 12, GlobalText[226], TRADE_WIDTH, 0, RT3_SORT_CENTER);

	//Nome do personagem que receberá e enviará os itens
	g_pRenderText->SetFont(g_hFontBold);
	g_pRenderText->SetTextColor(255, 255, 255, 200);
	g_pRenderText->RenderText(m_Pos.x+20, m_Pos.y+48, m_szYourID);
	// Nome do personagem da tela que enviará e visualizará os itens
	g_pRenderText->SetTextColor(255, 255, 255, 200);
	...
	g_pRenderText->RenderText(m_Pos.x + 110, m_Pos.y + 150 + 8, szTemp, 0, 0, RT3_WRITE_CENTER);

	::ConvertGold(m_nMyTradeGold, szTemp);
	g_pRenderText->SetTextColor(::getGoldColor(m_nMyTradeGold));
	g_pRenderText->RenderText(m_Pos.x + 110, m_Pos.y + 356 + 8, szTemp, 0, 0, RT3_WRITE_CENTER);

	g_pRenderText->SetTextColor(255, 220, 150, 255);
	g_pRenderText->RenderText(m_Pos.x + 28, m_Pos.y + 150 + 6, GlobalText[100], 0, 0, RT3_WRITE_CENTER);
	g_pRenderText->RenderText(m_Pos.x + 28, m_Pos.y + 356 + 6, GlobalText[100], 0, 0, RT3_WRITE_CENTER);

	g_pRenderText->SetTextColor(255, 220, 150, 255);
    g_pRenderText->RenderText(m_Pos.x+20,m_Pos.y+185,GlobalText[365]);
...
void CNewUITrade::LoadImages()
{
	...
	LoadBitmap("Custom\\NewInterface\\btn_medium.jpg", 150000);
}
...
bool CNewUITrade::ProcessBtns()
{
	...
	else if (SEASON3B::IsRelease(VK_LBUTTON) && CheckMouseIn(m_Pos.x + 146, m_Pos.y + 390, 24, 24))
	{
...
void CNewUITrade::DeleteMyTrade(int Slot)
{
	ITEM* pItemObj = m_pMyInvenCtrl->FindItem(Slot);
