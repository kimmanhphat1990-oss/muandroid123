...
bool SEASON3B::CNewUITextInputMsgBox::Create(DWORD dwMsgBoxType, DWORD dwInputType, int iInputBoxWidth, int iInputBoxHeight, int iTextLimit, bool bIsPassword)
{
	...
	x = (GetWindowsX / 2) - (MSGBOX_WIDTH / 2);
	y = 100;
	width = MSGBOX_WIDTH;
...
void SEASON3B::CNewUITextInputMsgBox::RenderTexts()
{
	...
	float x, y;
	x = GetPos().x; y = GetPos().y + MSGBOX_TEXT_TOP_BLANK - 15;
	type_vector_msgdata::iterator vi = m_MsgTextList.begin();
	for (; vi != m_MsgTextList.end(); vi++)
	{
...
void SEASON3B::CGemIntegrationMsgBox::SetButtonInfo()
{
	...
	height = MSGBOX_BTN_EMPTY_HEIGHT;
	m_BtnUnity.SetInfo(941101, x, y, 120, height, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY);

	m_BtnUnity.SetText(GlobalText[1801]);

	x = GetPos().x + msgboxhalfwidth - btnhalf;
	y += BTN_GAP;
	m_BtnDisjoint.SetInfo(941101, x, y, 120, height, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY);
	...
	y += BTN_GAP;
	width = MSGBOX_BTN_EMPTY_SMALL_WIDTH;
	m_BtnCancel.SetInfo(CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_EMPTY_SMALL, x + 12, y, width, height, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY_SMALL);
...
void SEASON3B::CGemIntegrationUnityMsgBox::SetButtonInfo()
{
	...
	{
		cButton.SetInfo(941102, x + 20.0f + (i % 2) * (20 + width), y + (height + 5.0f) * int(i / 2), width, height, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY_SMALL);
	...
	{
		cButton.SetInfo(941101, x + 50.0f, y + (height + 10.0f) * k, MSGBOX_BTN_EMPTY_WIDTH + 20, height, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY);
		// 1808 "%d°³ Á¶ÇÕ(%dÁ¨ ¼Ò¿ä)"
	...
	m_BtnCancel.SetInfo(CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_EMPTY_SMALL, x + 6, y, width, height, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY_SMALL);
	//m_BtnCancel.SetText(GlobalText[1002]);	
...
void SEASON3B::CSystemMenuMsgBox::SetButtonInfo()
{
	...
	width = MSGBOX_BTN_EMPTY_WIDTH + 25;
	height = MSGBOX_BTN_EMPTY_HEIGHT - 5;
	btnhalfwidth = width / 2.f;
	x = GetPos().x + msgboxhalfwidth - btnhalfwidth;
	y = GetPos().y + 23;
	m_BtnGameOver.SetInfo(931324, x, y, width, height, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY);
	m_BtnGameOver.SetText(GlobalText[381]);

	y += 30.f;
	m_BtnChooseServer.SetInfo(931324, x, y, width, height, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY);
	m_BtnChooseServer.SetText(GlobalText[382]);

	y += 30.f;
	m_BtnChooseCharacter.SetInfo(931324, x, y, width, height, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY);
	m_BtnChooseCharacter.SetText(GlobalText[383]);

	y += 30.f;
	m_BtnOption.SetInfo(931324, x, y, width, height, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY);
	m_BtnOption.SetText(GlobalText[385]);

	y += 30.f;
	m_BtnCancel.SetInfo(931324, x, y, width, height, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY);
...
CALLBACK_RESULT SEASON3B::CSystemMenuMsgBox::GameOverBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	...
	if (g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_MIXINVENTORY) || g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_CHAOSGENESIS))
	{
...
void SEASON3B::CChaosMixMenuMsgBox::SetButtonInfo()
{
	...
	btnhalfwidth = width / 2.f;
	x = GetPos().x + msgboxhalfwidth - btnhalfwidth;
	y = GetPos().y + 85 + PosXNew;
	m_BtnGeneralMix.SetInfo(931324, x, y, width, height, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY);
	m_BtnGeneralMix.SetText(GlobalText[735]);

	y = GetPos().y + 155 + PosXNew;
	m_BtnChaosMix.SetInfo(931324, x, y, width, height, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY);
	...
	x = GetPos().x + msgboxhalfwidth - btnhalfwidth;
	y = GetPos().y + GetSize().cy - (MSGBOX_BTN_EMPTY_HEIGHT + MSGBOX_BTN_BOTTOM_BLANK);
	m_BtnCancel.SetInfo(931325, x, y, width, height, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY_SMALL);
}
...
void SEASON3B::CChaosMixMenuMsgBox::RenderTexts()
{
	...
	g_pRenderText->RenderText(fPos_x, fPos_y + 10, szText, MSGBOX_WIDTH - 20.0f, 0, RT3_SORT_CENTER);
...
bool SEASON3B::CChangeVaultMsgBoxLayout::SetLayout()
{
	CNewUITextInputMsgBox* pMsgBox = GetMsgBox();
	if (0 == pMsgBox)
		return false;
	...
	pMsgBox->SetInputBoxOption(UIOPTION_NUMBERONLY | UIOPTION_PAINTBACK);
	pMsgBox->AddMsg(GlobalText[3159]);
	pMsgBox->AddCallbackFunc(CChangeVaultMsgBoxLayout::ReturnDown, MSGBOX_EVENT_PRESSKEY_RETURN);
	pMsgBox->AddCallbackFunc(CChangeVaultMsgBoxLayout::OkBtnDown, MSGBOX_EVENT_USER_COMMON_OK);
	pMsgBox->AddCallbackFunc(CChangeVaultMsgBoxLayout::CancelBtnDown, MSGBOX_EVENT_USER_COMMON_CANCEL);

	return true;
}

CALLBACK_RESULT SEASON3B::CChangeVaultMsgBoxLayout::ReturnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	return ProcessOk(pOwner, xParam);
}

CALLBACK_RESULT SEASON3B::CChangeVaultMsgBoxLayout::OkBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	return ProcessOk(pOwner, xParam);
}

CALLBACK_RESULT SEASON3B::CChangeVaultMsgBoxLayout::ProcessOk(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	CNewUITextInputMsgBox* pMsgBox = dynamic_cast<CNewUITextInputMsgBox*>(pOwner);
	unicode::t_char strText[MAX_TEXT_LENGTH] = { 0, };
	pMsgBox->GetInputBoxText(strText);
	...
	if (iInputZen < 1)
	{
		SEASON3B::CreateOkMessageBox(GlobalText[3160]);
	}
	else if (iInputZen > (int)CharacterMachine->WarehouseCount)
	{
		unicode::t_char newTExt[MAX_TEXT_LENGTH] = { 0, };
		unicode::_sprintf(newTExt, GlobalText[3161], (int)CharacterMachine->WarehouseCount);
		SEASON3B::CreateOkMessageBox(newTExt);
	}
	else if (iInputZen == (int)CharacterMachine->CurrentWarehouse)
	{
		SEASON3B::CreateOkMessageBox(GlobalText[3162]);
	}
	else
	{
		g_pNewUISystem->Hide(SEASON3B::INTERFACE_STORAGE);
		SendRequestChangeWare(iInputZen);
	}
...
CALLBACK_RESULT SEASON3B::CChangeVaultMsgBoxLayout::CancelBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	PlayBuffer(SOUND_CLICK01);
	g_MessageBox->SendEvent(pOwner, MSGBOX_EVENT_DESTROY);
...
void SEASON3B::CGuild_ToPerson_Position::SetButtonInfo()
{
	...
	y = GetPos().y + 50;
	m_BtnBlessing.SetInfo(941101, x, y, width, height, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY_SMALL);
	m_BtnBlessing.SetText(GlobalText[1311]);

	y += 27;
	m_BtnSoul.SetInfo(941101, x, y, width, height, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY_SMALL);
	...
	x -= 9;
	y += 70;
	m_BtnOk.SetInfo(931327, x, y, width, height, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY_SMALL);

	width = MSGBOX_BTN_EMPTY_SMALL_WIDTH;
	btnhalfwidth = width / 2.f;
	x += 64;
	m_BtnCancel.SetInfo(931326, x + 16, y, width, height, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY_SMALL);
}
...
void SEASON3B::CGuild_ToPerson_Position::RenderTexts()
{
	...
	x = GetPos().x; y = (GetPos().y + (MSGBOX_TEXT_TOP_BLANK / 2)) + 90;
