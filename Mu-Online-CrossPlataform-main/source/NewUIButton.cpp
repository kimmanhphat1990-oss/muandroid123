...
bool SEASON3B::CNewUIButton::Render( bool RendOption )
{
	if( m_ButtonInfo.size() != 0 )
	{
		if (m_CurImgIndex == 33028 || m_CurImgIndex == 33029)
		{
			switch (m_CurImgState)
			{
			case 0:
				RenderBitmap(m_CurImgIndex, m_Pos.x, m_Pos.y, m_Size.x, m_Size.y, 0.008, 0.002, 0.83, 0.20, 1, 1, 1.0); // IMAGEM INICIAL
				break;
			case 1:
				RenderBitmap(m_CurImgIndex, m_Pos.x, m_Pos.y, m_Size.x, m_Size.y, 0.008, 0.213, 0.83, 0.20, 1, 1, 1.0);
				break;
			case 2:
				RenderBitmap(m_CurImgIndex, m_Pos.x, m_Pos.y, m_Size.x, m_Size.y, 0.008, 0.429, 0.83, 0.20, 1, 1, 1.0);	// QUANDO � PRESSIONADO
				break;
			}
		}
		else if (m_CurImgIndex == 931325 || m_CurImgIndex == 931327)
		{
			GWidescreen.RenderImages(m_CurImgIndex, m_Pos.x, m_Pos.y, 54, 23, 0.0, m_CurImgState * 54.f, 122.f, 53.5f);
		}
		else
		{
			if (RendOption == true)
	...
	if( m_TooltipText.size() != 0 )
	{
		if( CheckMouseIn(m_Pos.x, m_Pos.y, m_Size.x, m_Size.y) )
		{
			...
			x = ( _iTempWidth > GWidescreen.JCWinWidth) ? (x - ( _iTempWidth - GWidescreen.JCWinWidth)) : x;

			if( m_IsTopPos ) y = m_Pos.y-(Fontsize.cy+2);

#ifdef NEW_MUHELPER_ON
			RenderText(m_TooltipText.c_str(), x + m_iMoveTipPosX, y + m_iMoveTipPosY, Fontsize.cx + 6, 0, m_hToolTipFont, m_TooltipTextColor, RGBA(0, 0, 0, 180), RT3_SORT_CENTER);
#else
