...
void CServerSelWin::Create()
{
	...
	CWin::SetSize((SERVER_GROUP_BTN_WIDTH + SSW_GAP_WIDTH) * 2 +SERVER_BTN_WIDTH + 150,SERVER_BTN_HEIGHT*SSW_SERVER_MAX + SSW_GAP_HEIGHT*2 + SERVER_GROUP_BTN_HEIGHT+m_winDescription.GetHeight());
}
...
void CServerSelWin::SetPosition(int nXCoord, int nYCoord)
{
	...
	int nServerGBtnBasePosY = nYCoord + CWin::GetHeight() - (nServerGBtnHeight * 11 + SSW_GAP_HEIGHT * 2 + nDescGgHeight) + 25;
	...
	for (i=0 ; i<SSW_LEFT_SERVER_G_MAX; i++)
	{
		...
		m_aServerGroupBtn[icntServreGroup++].SetPosition(nXCoord + (CWin::GetWidth() - nServerGBtnWidth) / 2, nBtnPosY);
	}
