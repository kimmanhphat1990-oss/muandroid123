...
namespace
{
    void SetBlurSparkVertex(platform::Vertex3D* vertex, const vec3_t position, float u, float v, float r, float g, float b)
    {
        vertex->x = position[0];
        vertex->y = position[1];
        vertex->z = position[2];
    ...
    void DrawBlurSegmentQuad(const vec3_t a, const vec3_t b, const vec3_t c, const vec3_t d, float u0, float u1, const vec3_t light0, const vec3_t light1)
    {
        platform::Vertex3D quad[4];
        SetBlurSparkVertex(&quad[0], a, u0, 1.f, light0[0], light0[1], light0[2]);
        SetBlurSparkVertex(&quad[1], b, u0, 0.f, light0[0], light0[1], light0[2]);
        SetBlurSparkVertex(&quad[2], c, u1, 0.f, light1[0], light1[1], light1[2]);
        SetBlurSparkVertex(&quad[3], d, u1, 1.f, light1[0], light1[1], light1[2]);
            ...
            if (b->Number >= 2)
            {
                ...
                for (int j = 0; j < b->Number - 1; j++)
                {
                    ...
                    if (b->Owner->Level == 0)
                        ...
                        Light = 1.f;
                    Vector(b->Light[0] * Light, b->Light[1] * Light, b->Light[2] * Light, light0);
                    TexU = (j) / (float)b->Number;
                    if (b->Owner->Level == 0)
                        Light = (b->Number - (j + 1)) / (float)b->Number;
                    else
                        Light = 1.f;
                    Vector(b->Light[0] * Light, b->Light[1] * Light, b->Light[2] * Light, light1);
                    DrawBlurSegmentQuad(b->p1[j], b->p2[j], b->p2[j + 1], b->p1[j + 1], TexU, (j + 1) / (float)b->Number, light0, light1);
                }
            }
        }
    }
...
void CreateBlood(OBJECT* o)
{
    ...
    if (BoneHead != -1)
    {
        ...
        else if (o->Type != MODEL_GHOST && o->Type != MODEL_MONSTER01 + 14 && o->Type != MODEL_MONSTER01 + 18)
        {
            vec3_t p, Position;
            for (int i = 0; i < 2; i++)
            {
...
void RenderFlagFace(OBJECT* o, int x, int y, vec3_t Light, int Tex1, int Tex2)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        SetBlurSparkVertex(&quad[i], Position, TexCoord[i][0], TexCoord[i][1], Light[0] * v->light, Light[1] * v->light, Light[2] * v->light);
    }
    platform::GetRenderBackend().DrawQuad3D(quad, true, true);

    BindTexture(Tex1);
    platform::Vertex3D reverse_quad[4];
    int quad_index = 0;
    for (int i = n - 1; i >= 0; i--)
    {
        int vlist = f->vlist[i];
        ...
        Vector(v->p[0] + 9.f, v->p[1] - 12.f, v->p[2] - 35.f, p);
        Models[o->Type].TransformPosition(o->BoneTransform[19], p, Position, true);
        SetBlurSparkVertex(&reverse_quad[quad_index], Position, TexCoord[i][0], TexCoord[i][1], Light[0] * v->light, Light[1] * v->light, Light[2] * v->light);
        quad_index++;
    }
    platform::GetRenderBackend().DrawQuad3D(reverse_quad, true, true);
}
