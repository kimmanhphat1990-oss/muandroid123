...
void CNewUIInGameShop::RenderDisplayItems()
{
	...
	BeginModelPreviewViewport(0, 0, WindowWidth, WindowHeight, 1.9f, RENDER_ITEMVIEW_NEAR, RENDER_ITEMVIEW_FAR, true);

	for (int i = 0; i < g_InGameShopSystem->GetSizePackageAsDisplayPackage(); i++)
	{
		int iPosX = GetCenterX(640) + IGS_ITEMRENDER_POS_X_STANDAD + (IMAGE_IGS_VIEWDETAIL_BTN_DISTANCE_X * (i % IGS_NUM_ITEMS_WIDTH));
...
bool CNewUIInGameShop::BtnProcess()
{
	...
	if (m_UseButton.UpdateMouseEvent() == true)
	{
		...
		if (iStorageIndex == IGS_SAFEKEEPING_LISTBOX)					// º¸°üÇÔ
		{
			...
			pMsgBox->Initialize(pSelectItem->m_iStorageSeq, pSelectItem->m_iStorageItemSeq, pSelectItem->m_wItemCode, pSelectItem->m_szType, pSelectItem->m_szName, pSelectItem->m_szNum, pSelectItem->m_szPeriod);
		}
		else if (iStorageIndex == IGS_PRESENTBOX_LISTBOX)				// ¼±¹° º¸°üÇÔ
		{
			...
			pMsgBox->Initialize(pSelectItem->m_iStorageSeq, pSelectItem->m_iStorageItemSeq, pSelectItem->m_wItemCode, pSelectItem->m_szType, pSelectItem->m_szSendUserName, pSelectItem->m_szMessage, pSelectItem->m_szName, pSelectItem->m_szNum, pSelectItem->m_szPeriod);
		}
		return true;
	}
...
void CNewUIInGameShop::SetBtnInfo()
{
	...
	for (int i = 0; i < INGAMESHOP_DISPLAY_ITEMLIST_SIZE; i++)
	{
		...
		m_ViewDetailButton[i].ChangeButtonInfo(PosX + IMAGE_IGS_VIEWDETAIL_BTN_POS_X + ((i % IGS_NUM_ITEMS_WIDTH) * IMAGE_IGS_VIEWDETAIL_BTN_DISTANCE_X), IMAGE_IGS_VIEWDETAIL_BTN_POS_Y + ((i / IGS_NUM_ITEMS_HEIGHT) * IMAGE_IGS_VIEWDETAIL_BTN_DISTANCE_Y), IMAGE_IGS_VIEWDETAIL_BTN_WIDTH, IMAGE_IGS_VIEWDETAIL_BTN_HEIGHT);
...
void CNewUIInGameShop::RenderBanner()
{
	...
	RenderImage(IMAGE_IGS_BANNER, GetCenterX(640) + IMAGE_IGS_BANNER_POS_X, IMAGE_IGS_BANNER_POS_Y, IMAGE_IGS_BANNER_WIDTH, IMAGE_IGS_BANNER_HEIGHT);
}
...
bool CNewUIInGameShop::UpdateBanner()
{
	...
	if ((SEASON3B::IsPress(VK_LBUTTON))
		&& (SEASON3B::CheckMouseIn(GetCenterX(640) + IMAGE_IGS_BANNER_POS_X, IMAGE_IGS_BANNER_POS_Y, IMAGE_IGS_BANNER_WIDTH, IMAGE_IGS_BANNER_HEIGHT)))
	{
