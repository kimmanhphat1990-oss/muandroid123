...
namespace
{
    void SetJointVertex(platform::Vertex3D* vertex, const vec3_t position, float u, float v)
    {
        vertex->x = position[0];
        vertex->y = position[1];
        vertex->z = position[2];
        ...
        if (o->Live && o->NumTails > 0 && o->RenderFace != 0)
        {
            ...
            for (int j = 0; j < (int)o->NumTails; j++)
            {
                ...
                if (o->Type == BITMAP_JOINT_FORCE && o->SubType == 0)
                {
                    ...
                    SetJointVertex(&quad[0], currentTail[0], Light1, 0.f);
                    SetJointVertex(&quad[1], currentTail[1], Light1, 1.f);
                    SetJointVertex(&quad[2], nextTail[1], Light2, 1.f);
                    SetJointVertex(&quad[3], nextTail[0], Light2, 0.f);
                    for (int vertex_index = 0; vertex_index < 4; ++vertex_index)
                    {
                        quad[vertex_index].r = Luminosity;
                        quad[vertex_index].g = Luminosity;
                        quad[vertex_index].b = Luminosity;
                        quad[vertex_index].a = 1.f;
                    }
                ...
                else
                {
                    ...
                    if (o->Type == BITMAP_FLARE && o->SubType == 22)
                    {
                        ...
                        platform::GetRenderBackend().TranslateModelView(t_bias[0], t_bias[1], t_bias[2]);

                        SetJointVertex(&quad[0], currentTail[2], Light1, 1.f);
                        SetJointVertex(&quad[1], currentTail[3], Light1, 0.f);
                        SetJointVertex(&quad[2], o->Tails[j + 1][3], Light2, 0.f);
                        SetJointVertex(&quad[3], o->Tails[j + 1][2], Light2, 1.f);
                        platform::GetRenderBackend().DrawQuad3D(quad, true, false);

                        SetJointVertex(&quad[0], currentTail[0], Light1, 0.f);
                        SetJointVertex(&quad[1], currentTail[1], Light1, 1.f);
                        SetJointVertex(&quad[2], o->Tails[j + 1][1], Light2, 1.f);
                        SetJointVertex(&quad[3], o->Tails[j + 1][0], Light2, 0.f);
                        platform::GetRenderBackend().DrawQuad3D(quad, true, false);
                    ...
                    {
                        platform::Vertex3D quad[4];
                        SetJointVertex(&quad[0], currentTail[2], L1, V2);
                        SetJointVertex(&quad[1], currentTail[3], L1, V1);
                        SetJointVertex(&quad[2], nextTail[3], L2, V1);
                        SetJointVertex(&quad[3], nextTail[2], L2, V2);
                        platform::GetRenderBackend().DrawQuad3D(quad, true, false);
                    }
                    ...
                    if ((o->RenderFace & RENDER_FACE_TWO) == RENDER_FACE_TWO)
                    {
                        ...
                        platform::Vertex3D quad[4];
                        SetJointVertex(&quad[0], currentTail[0], L1, V1);
                        SetJointVertex(&quad[1], currentTail[1], L1, V2);
                        SetJointVertex(&quad[2], nextTail[1], L2, V2);
                        SetJointVertex(&quad[3], nextTail[0], L2, V1);
                        platform::GetRenderBackend().DrawQuad3D(quad, true, false);
                    }
                }
            }
