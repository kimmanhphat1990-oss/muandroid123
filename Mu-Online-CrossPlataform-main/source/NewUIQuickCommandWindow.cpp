...
bool SEASON3B::CNewUIQuickCommandWindow::UpdateMouseEvent()
{
	...
	if(m_iSelectedIndex > -1 && SEASON3B::IsRelease(VK_LBUTTON))
	{
		...
		{
			CHARACTER* pCha = &CharactersClient[m_iSelectedCharacterIndex];
			g_pCommandWindow->CommandTradeX(pCha);
			CloseQuickCommand();
	...
	if(CheckMouseIn(m_Pos.x, m_Pos.y+30, 112, height) == false)
	{
		m_iSelectedIndex = -1;
	...
	if(CheckMouseIn(m_Pos.x, m_Pos.y, 112, 155))
	{
		return false;
	}
...
void SEASON3B::CNewUIQuickCommandWindow::RenderFrame()
{
	...
	x = m_Pos.x; y = m_Pos.y; width = 112.f; height = 155;
...
void SEASON3B::CNewUIQuickCommandWindow::RenderContents()
{
	...
	int iGlobalText[] = { 943, 1124, 944, 948, 949, 4000};
	for(int i = 0; i < 6; ++i)
	{
