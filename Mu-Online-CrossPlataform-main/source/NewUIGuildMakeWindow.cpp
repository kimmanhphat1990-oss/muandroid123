...
bool CNewUIGuildMakeWindow::Create(CNewUIManager* pNewUIMng, int x, int y)
{
	...
	for( int i = 0; i < GUILDMAKEBUTTON_COUNT; ++i )
	{
		...
		{
			m_Button[i].ChangeButtonImgState(true, 931326, true);
			m_Button[i].ChangeButtonInfo(0, 0, 64, 29);
		}
		else if (i == 2 || i == 4)
		{
			m_Button[i].ChangeButtonImgState(true, 931327, true);
			m_Button[i].ChangeButtonInfo(0, 0, 64, 29);
		}
	}
...
bool CNewUIGuildMakeWindow::UpdateGMMark()
{
	...
	//button
	m_Button[GUILDMAKEBUTTON_MARK_LNEXT].SetPos(m_Pos.x+25, m_Pos.y+379);
...
{
	m_Button[GUILDMAKEBUTTON_RESULTINFO_LNEXT].SetPos(m_Pos.x+25, m_Pos.y+379);
...
void CNewUIGuildMakeWindow::RenderGMMark()
{
	...
	RenderText( Text, m_Pos.x+15, m_Pos.y+66, 190, 0, 0xFF49B0FF, 0x00000000, RT3_SORT_LEFT );

	GWidescreen.RenderImages(931316, m_Pos.x + 45, m_Pos.y + 60, 114.f, 21.f, 0.0, 0.0, 256.f, 46.5f);
	m_EditBox->Render();
