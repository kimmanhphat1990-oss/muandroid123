...
//					Vector(0.7f,0.5f,0.8f,b->BodyLight);
            ...
            else if ( o->Type==MODEL_MONSTER01+60 )
            {
                if (  o->CurrentAction==MONSTER01_DIE || 
					( MoveSceneFrame - o->InitialSceneTime) < 25
					)
                {
					if ( o->CurrentAction==MONSTER01_DIE)
					{
...
void RenderBoundingBox(OBJECT* pObj)
{
    ...
    platform::Vertex3D line_vertices[24];
    for (int i = 0; i < 12; ++i)
    {
        const float* color = line_colors[i / 2];
        for (int vertex_index = 0; vertex_index < 2; ++vertex_index)
        {
            const vec3_t& source = TransformVertices[line_indices[i][vertex_index]];
            platform::Vertex3D& vertex = line_vertices[i * 2 + vertex_index];
            vertex.x = source[0];
            vertex.y = source[1];
            vertex.z = source[2];
            vertex.u = 0.0f;
    ...
    platform::GetRenderBackend().DrawLineList3D(line_vertices, 24, true);
