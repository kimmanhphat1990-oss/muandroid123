...
void CCameraMove::RenderWayPoint()
{
	...
	for(; iter != m_listWayPoint.end(); iter++) {
		...
		quad[0] = { pWayPoint->fCameraX+50-10, pWayPoint->fCameraY+50-10, pWayPoint->fCameraZ, 0.f, 0.f, 1.f, 0.f, 0.f, 0.8f };
		quad[1] = { pWayPoint->fCameraX+50+10, pWayPoint->fCameraY+50-10, pWayPoint->fCameraZ, 0.f, 0.f, 1.f, 0.f, 0.f, 0.8f };
		quad[2] = { pWayPoint->fCameraX+50+10, pWayPoint->fCameraY+50+10, pWayPoint->fCameraZ, 0.f, 0.f, 1.f, 0.f, 0.f, 0.8f };
		quad[3] = { pWayPoint->fCameraX+50-10, pWayPoint->fCameraY+50+10, pWayPoint->fCameraZ, 0.f, 0.f, 1.f, 0.f, 0.f, 0.8f };
		platform::GetRenderBackend().DrawQuad3D(quad, false, true);
	}

	if (m_listWayPoint.size() >= 2)
	{
		std::vector<platform::Vertex3D> line_vertices;
		line_vertices.reserve((m_listWayPoint.size() - 1) * 2);
		iter = m_listWayPoint.begin();
		WAYPOINT* previous = (*iter);
		++iter;
		for(; iter != m_listWayPoint.end(); ++iter) {
			WAYPOINT* current = (*iter);
			line_vertices.push_back({ previous->fCameraX+50, previous->fCameraY+50, previous->fCameraZ, 0.f, 0.f, 1.f, 1.f, 1.f, 0.5f });
			line_vertices.push_back({ current->fCameraX+50, current->fCameraY+50, current->fCameraZ, 0.f, 0.f, 1.f, 1.f, 1.f, 0.5f });
			previous = current;
		}
		platform::GetRenderBackend().DrawLineList3D(line_vertices.data(), static_cast<int>(line_vertices.size()), true);
	}
