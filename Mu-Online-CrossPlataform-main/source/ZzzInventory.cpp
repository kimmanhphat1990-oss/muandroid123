...
void ComputeItemInfo(int iHelpItem)
{
	...
	for(int Level = 0; Level <= iMaxLevel; Level++)
	{
		...
		DWORD Strength, Dexterity, Energy, Vitality, Charisma;
...
void GetItemName ( int iType, int iLevel, char* Text )
{
	...
	else if ((iType >= ITEM_WING + 36 && iType <= ITEM_WING + 40) || (iType >= ITEM_WING + 42 && iType <= ITEM_WING + 43 || gCustomWing.CheckCustomWingByItem(iType)))
    {
		if(iLevel==0)
			sprintf(Text,"%s",p->Name);
		else
...
//			TextBold[TextNum] = true;
	...
	else if ((ip->Type>=ITEM_WING+36 && ip->Type<=ITEM_WING+40) || (ip->Type>=ITEM_WING+42 && ip->Type<=ITEM_WING+43 || gCustomWing.CheckCustomWingByItem(ip->Type))
	...
	//description contador da morte
	if (gCounterItem.CheckIsItem(ip->Type)) 
	{
		int count = 0;

		int isTop = 0;

		gCounterItem.getCount(ip->x, ip->y, &count, &isTop);

		auto itemInfo = gCounterItem.getItemInfo(ip->Type);

		if (!strcmp(g_aszMLSelection, "Por"))
		{
			sprintf(TextList[TextNum], itemInfo->TextPor, count);
		}
		else if (!strcmp(g_aszMLSelection, "Eng"))
		{
			sprintf(TextList[TextNum], itemInfo->TextEng, count);
		}
		else if (!strcmp(g_aszMLSelection, "Spn"))
		{
			sprintf(TextList[TextNum], itemInfo->TextSpn, count);
		}
		else
		{
			sprintf(TextList[TextNum], itemInfo->TextPor, count);
		...
		if (isTop) {
			TextListColor[TextNum] = 114; TextBold[TextNum] = itemInfo->Bold; TextNum++;
		}
		else
		{
			TextListColor[TextNum] = itemInfo->Color; TextBold[TextNum] = itemInfo->Bold; TextNum++;
		}
    ...
    {
		sprintf(TextList[TextNum],GlobalText[577],32+Level);  //  µ¥¹ÌÁö ¸î%Áõ°¡.
		TextListColor[TextNum] = TEXT_COLOR_WHITE;TextBold[TextNum] = false;TextNum++;
		sprintf(TextList[TextNum],GlobalText[578],25+Level*2);  //  µ¥¹ÌÁö ¸î%Èí¼ö.
		TextListColor[TextNum] = TEXT_COLOR_WHITE;TextBold[TextNum] = false;TextNum++;
		sprintf(TextList[TextNum],GlobalText[579]);             //  ÀÌµ¿ ¼Óµµ Çâ»ó.
		TextListColor[TextNum] = TEXT_COLOR_WHITE;TextBold[TextNum] = false;TextNum++;
    }
    ...
    {
		sprintf(TextList[TextNum],GlobalText[577], gCustomWing.GetCustomWingIncDamage(ip->Type, Level));
		TextListColor[TextNum] = TEXT_COLOR_WHITE;TextBold[TextNum] = false;TextNum++;
		sprintf(TextList[TextNum],GlobalText[578], gCustomWing.GetCustomWingDecDamage(ip->Type, Level));		
		TextListColor[TextNum] = TEXT_COLOR_WHITE;TextBold[TextNum] = false;TextNum++;
		sprintf(TextList[TextNum],GlobalText[579]);
		TextListColor[TextNum] = TEXT_COLOR_WHITE;TextBold[TextNum] = false;TextNum++;
    }
	...
	{
		// ¸ÁÅä °ü·Ã ¿É¼Çº¯°æ
		sprintf(TextList[TextNum],GlobalText[577],20+Level*2);  //  µ¥¹ÌÁö ¸î%Áõ°¡
		TextListColor[TextNum] = TEXT_COLOR_WHITE;TextBold[TextNum] = false;TextNum++;
		int _iDamage = (ip->Type==ITEM_WING+49) ? 10+Level*2 : 10+Level;
		sprintf(TextList[TextNum],GlobalText[578],_iDamage);  //  µ¥¹ÌÁö ¸î%Èí¼ö
		TextListColor[TextNum] = TEXT_COLOR_WHITE;TextBold[TextNum] = false;TextNum++;
	}
	...
	if ( ( p->Durability || p->MagicDur ) && 
		...
		|| (ip->Type>=ITEM_WING+36 && ip->Type<=ITEM_WING+43 || gCustomWing.CheckCustomWingByItem(ip->Type))
#ifdef PBG_ADD_NEWCHAR_MONK_ITEM
		|| (ip->Type>=ITEM_WING+49 && ip->Type<=ITEM_WING+50)
#endif //PBG_ADD_NEWCHAR_MONK_ITEM
       )
      ...
      )
	{
		int Success = false;
        int arrow = false;
		if (ip->Type >= ITEM_POTION && ip->Type <= ITEM_POTION + 8 || gJewelStack.CheckIsStack(ip->Type) && ip->Durability > 1)
		{
          	sprintf(TextList[TextNum],GlobalText[69],ip->Durability);
			Success = true;
		}
	...
	if(ip->RequireVitality && bRequireStat ) //  ¿ä±¸Ã¼·Â.
	{
	...
	else if(g_pMyInventory->IsInvenItem(ip->Type))
	{
		...
		switch (ip->Type)
		{
			...
			case ITEM_HELPER+132:
				...
				TextNum++;
				break;
			case ITEM_HELPER+133:	// °ñµç¸ÞÀÌÇÃÂü
	...
	TextNum = gDescriptions.DescriptionsCall(TextNum, ip);

	if (Visuals.VisualOriginalDescription == 0)
	{
		if (p->m_byItemSlot >= 240 && p->m_byItemSlot <= 248)
		{
			TextNum = Visuals.DescriptionsCall(TextNum, ip);
		}
	}
...
void RenderRepairInfo(int sx,int sy,ITEM *ip,bool Sell)
{
	...
	if(ip->Type==ITEM_POTION+13 || ip->Type==ITEM_POTION+14 || ip->Type==ITEM_WING+15 || gCustomJewel.CheckCustomJewelByItem(ip->Type))
	{
		Color = TEXT_COLOR_YELLOW;
	}
...
void RenderItemName(int i,OBJECT *o,int ItemLevel,int ItemOption,int ItemExtOption,bool Sort)
{
	...
	GLfloat fCurColor[4] = { 1.f, 1.f, 1.f, 1.0f };
...
int GetScreenWidth()
{
	...
	return GWidescreen.JCWinWidth - iWidth;
}
...
void InsertInventory(ITEM *Inv,int Width,int Height,int Index,BYTE *Item,bool First)
{
	...
	if(Inv==Inventory)
	{
		if(Index < 12)
		{
			...
			if ( Index==EQUIPMENT_WING || Index == NEW_EQUIPMENT_WING)
			{
				if (Type==ITEM_WING+39 || 
					...
					Type==ITEM_WING+135||
#endif //PBG_ADD_NEWCHAR_MONK_ITEM
					Type==ITEM_WING+40 || gCustomWing.CheckCustomWingIsCape(Type) > 0)
				{
...
bool IsHighValueItem(ITEM* pItem)
{
	...
	if(
		...
		(pItem->Type >= ITEM_WING+36 && pItem->Type <= ITEM_WING+43 || gCustomWing.CheckCustomWingByItem(pItem->Type)) ||
...
{
	if (gCustomWing.CheckCustomWingByItem(pItem->Type))
	{
		return true;
	}
...
{	
	...
	float ModelScale = (0.1f * RateH);

	float ModelScalePick = (0.07f * RateH);
	...
	if (PickUp)
		VectorMA(MousePosition, (0.075f * RateH), Direction, Position);
	else
		VectorMA(MousePosition, (0.15f * RateH), Direction, Position);
	//if(PickUp)
 //     	VectorMA(MousePosition,0.08f,Direction,Position);
	//else
 //     	VectorMA(MousePosition,0.14f,Direction,Position);
	...
	else if (gCustomItemPosition.CheckCustomItemPosition(Type - MODEL_ITEM))
	{
		gCustomItemPosition.GetPositionItem(Type - MODEL_ITEM, &PosX, &PosY, &AngleX, &AngleY, &AngleZ);

		Position[0] += PosX;
		Position[1] += PosY;
		Vector(AngleX, AngleY, AngleZ, ObjectSelect.Angle);
	}
...
//		Position[0] += 0.02f;
	...
	if(1==Select)
	{
		ObjectSelect.Angle[1] = WorldTime*0.25f;
	}
	...
	if (gCustomItemSize.CheckCustomItemSize(Type - MODEL_ITEM))
	{
		static float Itemsize = 0.0;

		gCustomItemSize.GetSizeItem(Type - MODEL_ITEM, Itemsize);

		Scale = Itemsize;
	}
	else if(Type>=MODEL_HELM && Type<MODEL_BOOTS+MAX_ITEM_INDEX)
	{
		if(Type>=MODEL_HELM && Type<MODEL_HELM+MAX_ITEM_INDEX)			
		{
...
#endif //PBG_ADD_NEWCHAR_MONK_ITEM
#ifdef LEM_ADD_LUCKYITEM	// ·°Å°¾ÆÀÌÅÛ ÀÎº¥Åä¸® ½ºÄÉÀÏ ¼³Á¤ [lem_2010.9.7]
		// LEM_TSET  »ó½ÂÀÇ º¸¼®, ¿¬ÀåÀÇ º¸¼® ½ºÄÉÀÏ[lem_2010.9.7]
		else if(Type >= MODEL_HELPER+135 && Type <= MODEL_HELPER+145)
		{
			Scale = 0.001f;
		}
...
void RenderItem3D(float sx,float sy,float Width,float Height,int Type,int Level,int Option1,int ExtOption,bool PickUp)
{
	...
	if ((g_pPickedItem == NULL || PickUp) && SEASON3B::CheckMouseIn(sx, sy, Width, Height))
	{
		...
		{
			if (g_pNewUISystem->CheckMouseUse() == false || g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_INVENTORY))
				Success = true;
		}
	}
...
int CheckMouseOnKeyPad( void)
{
    ...
    Width = 213;Height = 2*5+6*40;WindowX = (GetWindowsX-Width)/2;WindowY = 60+40;//60 220
...
void CreateGuildMark( int nMarkIndex, bool blend )
{
    ...
    platform::LegacyCompatibleTexImage2D(GL_TEXTURE_2D, 0, b->Components, Width, Height, 0, GL_RGBA, GL_UNSIGNED_BYTE, b->Buffer);
}
...
void CreateCastleMark ( int Type, BYTE* buffer, bool blend )
{
	...
	for(int i=0;i<16;i++)
	{
		...
		case 4 :MarkColor[i] = (255<<24)+(  0<<16)+(  0<<8)+(255);break;//»¡
		case 5 :MarkColor[i] = (255<<24)+(  0<<16)+(128<<8)+(255);break;//
		case 6 :MarkColor[i] = (255<<24)+(  0<<16)+(255<<8)+(255);break;//³ë
		case 7 :MarkColor[i] = (255<<24)+(  0<<16)+(255<<8)+(128);break;//
		case 8 :MarkColor[i] = (255<<24)+(  0<<16)+(255<<8)+(  0);break;//ÃÊ
		case 9 :MarkColor[i] = (255<<24)+(128<<16)+(255<<8)+(  0);break;//
		case 10:MarkColor[i] = (255<<24)+(255<<16)+(255<<8)+(  0);break;//Ã»
		case 11:MarkColor[i] = (255<<24)+(255<<16)+(128<<8)+(  0);break;//
		case 12:MarkColor[i] = (255<<24)+(255<<16)+(  0<<8)+(  0);break;//ÆÄ
		case 13:MarkColor[i] = (255<<24)+(255<<16)+(  0<<8)+(128);break;//
		case 14:MarkColor[i] = (255<<24)+(255<<16)+(  0<<8)+(255);break;//º¸
		case 15:MarkColor[i] = (255<<24)+(128<<16)+(  0<<8)+(255);break;//
		}
	}
