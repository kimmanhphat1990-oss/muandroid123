...
namespace
{
    void SetBackendVertex3D(platform::Vertex3D* vertex, const vec3_t position, float u, float v, float r, float g, float b, float a)
    {
        vertex->x = position[0];
        vertex->y = position[1];
        vertex->z = position[2];
    ...
    void SetBackendCurrentColor(float r, float g, float b, float a)
    {
        platform::GetRenderBackend().SetCurrentColor(r, g, b, a);
    }

    void SetBackendCurrentColor3(const vec3_t color)
    {
        SetBackendCurrentColor(color[0], color[1], color[2], 1.0f);
    }
}
...
void BMD::BindLightMaps()
{
    ...
    for (int i = 0; i < NumLightMaps; i++)
    {
        ...
        if (lmp->Buffer != NULL)
        {
            ...
            platform::GetRenderBackend().SetTextureCombineMode(platform::TextureCombineMode_Modulate);
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
            platform::LegacyCompatibleTexImage2D(GL_TEXTURE_2D, 0, 3, lmp->Width, lmp->Height, 0, GL_RGB, GL_UNSIGNED_BYTE, lmp->Buffer);
        }
    }
    LightMapEnable = true;
}
...
{
    platform::GetRenderBackend().PushModelView();
}
...
void BMD::EndRenderCoinHeap(int coinCount)
{
    ...
    constexpr int meshIndex = 0;
    Mesh_t* m = &Meshs[meshIndex];
    platform::GetRenderBackend().DrawPrimitiveArray3D(GL_TRIANGLES, &vertices[0][0], &texCoords[0][0], &colors[0][0],
        m->NumTriangles * 3 * coinCount, true, true);
}
...
void BMD::RenderMesh(int meshIndex, int renderFlags, float alpha, int blendMeshIndex, float blendMeshAlpha, float blendMeshTextureCoordU, float blendMeshTextureCoordV, int explicitTextureIndex)
{
    ...
    {
        SetBackendCurrentColor3(BodyLight);
        enableLight = false;
    }
    else if (enableLight)
    {
    ...
    if ((renderFlags & RENDER_COLOR) == RENDER_COLOR)
    {
        ...
        {
            EnableAlphaTest();
            SetBackendCurrentColor(BodyLight[0], BodyLight[1], BodyLight[2], alpha);
        }
    }
    ...
    else if (blendMeshIndex <= -2 || m->Texture == blendMeshIndex)
    {
        ...
        SetBackendCurrentColor(BodyLight[0] * blendMeshAlpha,
            BodyLight[1] * blendMeshAlpha,
            BodyLight[2] * blendMeshAlpha, 1.0f);
    ...
    platform::GetRenderBackend().DrawPrimitiveArray3D(GL_TRIANGLES, &vertices[0][0], &texCoords[0][0],
        enableColor ? &colors[0][0] : NULL, m->NumTriangles * 3, true, enableColor);
}
...
void BMD::RenderMeshAlternative(int iRndExtFlag, int iParam, int i, int RenderFlag, float Alpha, int BlendMesh, float BlendMeshLight, float BlendMeshTexCoordU, float BlendMeshTexCoordV, int MeshTexture)
{
    ...
    else if (BlendMesh <= -2 || m->Texture == BlendMesh)
    {
        ...
        SetBackendCurrentColor(BodyLight[0] * BlendMeshLight, BodyLight[1] * BlendMeshLight, BodyLight[2] * BlendMeshLight, 1.0f);
        //glColor3f(BlendMeshLight,BlendMeshLight,BlendMeshLight);
        EnableLight = false;
    }
    ...
    const bool enableColor = (EnableLight && Render == RENDER_TEXTURE) || Render == RENDER_CHROME;
    ...
    for (int j = 0; j < m->NumTriangles; j++)
    {
        ...
        for (int k = 0; k < tp->Polygon; k++)
        {
            ...
            if ((iRndExtFlag & RNDEXT_WAVE))
            {
                ...
                for (int iCoord = 0; iCoord < 3; ++iCoord)
                {
                    vertices[target_vertex_index][iCoord] = VertexTransform[i][vi][iCoord] + Normal[iCoord] * fSin * 28.0f;
                }
            }
            else
            {
                VectorCopy(VertexTransform[i][vi], vertices[target_vertex_index]);
            }
            ...
            case RENDER_TEXTURE:
            {
                ...
                texCoords[target_vertex_index][0] = texp->TexCoordU;
                texCoords[target_vertex_index][1] = texp->TexCoordV;
                if (EnableWave)
                {
                    texCoords[target_vertex_index][0] += BlendMeshTexCoordU;
                    texCoords[target_vertex_index][1] += BlendMeshTexCoordV;
                }
            ...
            case RENDER_CHROME:
            {
                texCoords[target_vertex_index][0] = g_chrome[ni][0];
                texCoords[target_vertex_index][1] = g_chrome[ni][1];
                break;
            }
            default:
            {
    ...
    platform::GetRenderBackend().DrawPrimitiveArray3D(GL_TRIANGLES, &vertices[0][0], &texCoords[0][0],
        enableColor ? &colors[0][0] : NULL, target_vertex_index + 1, true, enableColor);
}
        ...
        if (m->m_csTScript != NULL)
        {
            if (m->m_csTScript->getHiddenMesh() == false && i != HiddenMesh)
            {
                ...
                if (shadowType == SHADOW_RENDER_COLOR)
                {
                    DisableAlphaBlend();
                    if (Alpha >= 0.99f)
                        SetBackendCurrentColor(0.f, 0.f, 0.f, 1.0f);
                    else
                        SetBackendCurrentColor(0.f, 0.f, 0.f, Alpha);

                    RenderMesh(i, RENDER_COLOR | RENDER_SHADOWMAP, Alpha, iBlendMesh, BlendMeshLight, BlendMeshTexCoordU, BlendMeshTexCoordV);
                    SetBackendCurrentColor(1.f, 1.f, 1.f, 1.0f);
                }
...
void BMD::RenderMeshTranslate(int i, int RenderFlag, float Alpha, int BlendMesh, float BlendMeshLight, float BlendMeshTexCoordU, float BlendMeshTexCoordV, int MeshTexture)
{
    ...
    for (int j = 0; j < m->NumTriangles; j++)
    {
        ...
        for (int k = 0; k < tp->Polygon; k++)
        {
            ...
            VectorAdd(VertexTransform[i][vi], BodyOrigin, vertices[target_vertex_index]);
            Vector4(BodyLight[0], BodyLight[1], BodyLight[2], Alpha, colors[target_vertex_index]);
...
void BMD::AddClothesShadowTriangles(void* pClothes, const int clothesCount, const float sx, const float sy) const
{
    ...
    platform::GetRenderBackend().DrawPrimitiveArray3D(GL_TRIANGLES, &vertices[0][0], NULL, NULL, target_vertex_index + 1, false, false);
}
...
void BMD::RenderBodyShadow(const int blendMesh, const int hiddenMesh, const int startMeshNumber, const int endMeshNumber, void* pClothes, const int clothesCount)
{
    ...
    SetBackendCurrentColor(0.0f, 0.0f, 0.0f, 0.5f); // 50% opacity for shadows
    ...
    for (int i = 0; i < NumBones; i++)
    {
        ...
        if (b->BoundingBox)
        {
            ...
            for (int j = 0; j < 8; j++)
            {
                ...
                VectorMA(BodyOrigin, BodyScale, BoundingVertices[j], BoundingVertices[j]);
            }
            ...
            for (int face = 0; face < 6; ++face)
            {
                ...
                {
                    const int source_index = kFaceIndices[face][vertex_index];
                    SetBackendVertex3D(
                        &quad[vertex_index],
                        BoundingVertices[source_index],
...
{
    DisableTexture();
    platform::GetRenderBackend().SetDepthFunction(GL_ALWAYS);
    for (int i = 0; i < NumBones; i++)
    {
        Bone_t* b = &Bones[i];
        if (!b->Dummy)
        {
            ...
            if (Parent > 0)
            {
                ...
                SetBackendVertex3D(&line_vertices[0], BoneVertices[0], 0.f, 0.f, 0.8f, 0.8f, 0.2f, 1.f);
                SetBackendVertex3D(&line_vertices[1], BoneVertices[1], 0.f, 0.f, 0.8f, 0.8f, 0.2f, 1.f);
                SetBackendVertex3D(&line_vertices[2], BoneVertices[1], 0.f, 0.f, 0.8f, 0.8f, 0.2f, 1.f);
                SetBackendVertex3D(&line_vertices[3], BoneVertices[2], 0.f, 0.f, 0.8f, 0.8f, 0.2f, 1.f);
                SetBackendVertex3D(&line_vertices[4], BoneVertices[2], 0.f, 0.f, 0.8f, 0.8f, 0.2f, 1.f);
                SetBackendVertex3D(&line_vertices[5], BoneVertices[0], 0.f, 0.f, 0.8f, 0.8f, 0.2f, 1.f);
                platform::GetRenderBackend().DrawLineList3D(line_vertices, 6, true);
            }
        }
    }
    platform::GetRenderBackend().SetDepthFunction(GL_LEQUAL);
}
...
{
    std::string model_name = platform::ResolveGameAssetPath((std::string(DirName) + ModelFileName).c_str());
    ...
    Meshs = new Mesh_t[std::max<int>(1, NumMeshs)];
    Bones = new Bone_t[std::max<int>(1, NumBones)];
    Actions = new Action_t[std::max<int>(1, NumActions)];
    Textures = new Texture_t[std::max<int>(1, NumMeshs)];
    IndexTexture = new GLuint[std::max<int>(1, NumMeshs)];
