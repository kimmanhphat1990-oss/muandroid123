...
void CCharSelMainWin::Create()
{
	...
	for (int i = 0; i < ResolveCharacterSceneMaxCharacters(); ++i)
	{
...
void CCharSelMainWin::UpdateDisplay()
{
	...
	for (i = 0; i < ResolveCharacterSceneMaxCharacters(); ++i)
	{
		if (!CharactersClient[i].Object.Live)
		{
...
{
	if (nIndex < 0 || nIndex >= CSMW_BTN_MAX)
	{
		return NULL;
	}
...
const CSprite* CCharSelMainWin::GetBackSprite(int nIndex) const
{
	if (nIndex < 0 || nIndex >= CSMW_SPR_MAX)
	{
		return NULL;
	}
