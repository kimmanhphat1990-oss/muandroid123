...
SEASON3B::CNewUISystem::CNewUISystem()
{
	...
	m_GFxMacroMain = NULL;
	m_GFxMacroSubMain = NULL;
	m_GFxMacroGaugeBar = NULL;
#endif
...
bool SEASON3B::CNewUISystem::Create()
{
	...
	if(false == m_pNewChatLogWindow->Create(m_pNewUIMng, 0, setPosDown(415), 6))
		return false;

	m_pNewOptionWindow = new CNewUIOptionWindow;
	if(m_pNewOptionWindow->Create(m_pNewUIMng, (int)setPosCenterX(225), 70) == false)
	{
		return false;
	}
...
bool SEASON3B::CNewUISystem::LoadMainSceneInterface()
{
	...
	if(false == m_pNewChatInputBox->Create(m_pNewUIMng, m_pNewChatLogWindow, 0, setPosDown(415)))
	{
		return false;
	}
	...
	if(false == m_pNewNPCShop->Create(m_pNewUIMng, GWidescreen.WidescreenPosX2, CNewUINPCShop::NPCSHOP_POS_Y))
		return false;

	m_pNewPetInfoWindow = new CNewUIPetInfoWindow;
	if(false == m_pNewPetInfoWindow->Create(m_pNewUIMng, GWidescreen.WidescreenPosX2, 0))
		return false;
	
	m_pNewMixInventory = new CNewUIMixInventory;
	if(m_pNewMixInventory->Create(m_pNewUIMng, GWidescreen.WidescreenPosX2, 0) == false)
		return false;

	m_pNewCastleWindow = new CNewUICastleWindow;
	if(m_pNewCastleWindow->Create(m_pNewUIMng, GWidescreen.WidescreenPosX1, 0) == false)
		return false;

	m_pNewGuardWindow = new CNewUIGuardWindow;
	if(m_pNewGuardWindow->Create(m_pNewUIMng, GWidescreen.WidescreenPosX1, 0) == false)
		return false;

	m_pNewGatemanWindow = new CNewUIGatemanWindow;
	if(m_pNewGatemanWindow->Create(m_pNewUIMng, GWidescreen.WidescreenPosX1, 0) == false)
		return false;

	m_pNewGateSwitchWindow = new CNewUIGateSwitchWindow;
	if(m_pNewGateSwitchWindow->Create(m_pNewUIMng, GWidescreen.WidescreenPosX1, 0) == false)
		return false;

	m_pNewStorageInventory = new CNewUIStorageInventory;
	if(m_pNewStorageInventory->Create(m_pNewUIMng, GWidescreen.WidescreenPosX2, 0) == false)
		return false;

	m_pNewGuildInfoWindow = new CNewUIGuildInfoWindow;
	if(m_pNewGuildInfoWindow->Create(m_pNewUIMng, GWidescreen.WidescreenPosX1, 0) == false)
		return false;

	m_pNewGuildMakeWindow = new CNewUIGuildMakeWindow;
	if(m_pNewGuildMakeWindow->Create(m_pNewUIMng, GWidescreen.WidescreenPosX1, 0) == false)
		return false;

	CreatePersonalItemTable();

	m_pNewMyShopInventory = new CNewUIMyShopInventory;
	if(m_pNewMyShopInventory->Create(m_pNewUIMng, GWidescreen.WidescreenPosX2, 0) == false)
		return false;
	
	m_pNewPurchaseShopInventory = new CNewUIPurchaseShopInventory;
	if(m_pNewPurchaseShopInventory->Create(m_pNewUIMng, GWidescreen.WidescreenPosX2, 0) == false)
		return false;

	m_pNewCharacterInfoWindow = new CNewUICharacterInfoWindow;
	if(m_pNewCharacterInfoWindow->Create(m_pNewUIMng, GWidescreen.WidescreenPosX1, 0) == false)
		return false;

	m_pNewMyQuestInfoWindow = new CNewUIMyQuestInfoWindow;
	if(m_pNewMyQuestInfoWindow->Create(m_pNewUIMng, GWidescreen.WidescreenPosX1, 0) == false)
		return false;

	m_pNewPartyInfoWindow = new CNewUIPartyInfoWindow;
	if( m_pNewPartyInfoWindow->Create( m_pNewUIMng, GWidescreen.WidescreenPosX1, 0 ) == false )
		return false;

	m_pNewPartyListWindow = new CNewUIPartyListWindow;
	if( m_pNewPartyListWindow->Create( m_pNewUIMng, GetWindowsX - 79, 2 ) == false )
		return false;

	m_pNewNPCQuest = new CNewUINPCQuest;
	if (m_pNewNPCQuest->Create(m_pNewUIMng, m_pNewUI3DRenderMng, GWidescreen.WidescreenPosX1, 0) == false)
		return false;

	m_pNewEnterBloodCastle = new CNewUIEnterBloodCastle;
	if( m_pNewEnterBloodCastle->Create(m_pNewUIMng, GWidescreen.WidescreenPosX1, 0) == false )
		return false;

	m_pNewEnterDevilSquare = new CNewUIEnterDevilSquare;
	if( m_pNewEnterDevilSquare->Create(m_pNewUIMng, GWidescreen.WidescreenPosX1, 0) == false )
		return false;
	
	m_pNewBloodCastle = new CNewUIBloodCastle;
	if( m_pNewBloodCastle->Create(m_pNewUIMng, GetWindowsX - 127, 480-132) == false )
		return false;

	m_pNewTrade = new CNewUITrade;
	if (m_pNewTrade->Create(m_pNewUIMng, GWidescreen.WidescreenPosX2, 0) == false)
		return false;
	...
	if(m_pNewKanturuInfoWindow->Create(m_pNewUIMng, (GetWindowsX - 99), 351) == false)
	{
		return false;
	}

	m_pNewChaosCastleTime = new CNewUIChaosCastleTime;
	if( m_pNewChaosCastleTime->Create(m_pNewUIMng, GetWindowsX - 127, 480-132) == false )
		return false;

	m_pNewBattleSoccerScore = new CNewUIBattleSoccerScore;
	if (m_pNewBattleSoccerScore->Create(m_pNewUIMng, GetWindowsX - 131, 359) == false)
		return false;
	
	m_pNewCommandWindow = new CNewUICommandWindow;
	if( m_pNewCommandWindow->Create(m_pNewUIMng, GWidescreen.WidescreenPosX1, 0) == false )
		return false;

	m_pNewCatapultWindow = new CNewUICatapultWindow;
	if(m_pNewCatapultWindow->Create(m_pNewUIMng, GWidescreen.WidescreenPosX1, 0) == false)
	{
		return false;
	}

	m_pNewWindowMenu = new CNewUIWindowMenu;
	if (m_pNewWindowMenu->Create(m_pNewUIMng, (GetWindowsX - 112), 480 - 171) == false)
	{
		return false;
	}
	...
	m_GFxMacroMain = new CGFxMacroMain;

	if (m_GFxMacroMain->Create(m_pNewUIMng, GetWindowsX - 190, 0) == false)
	{
		return false;
	}

	m_GFxMacroSubMain = new CGFxMacroSubMain;

	if (m_GFxMacroSubMain->Create(m_pNewUIMng, GetWindowsX - 380, 0) == false)
	{
		return false;
	}

	m_GFxMacroGaugeBar = new CGFxMacroGaugeBar;
	
	if (m_GFxMacroGaugeBar->Create(m_pNewUIMng, 0, 14) == false)
	{
		return false;
	}
#else
	...
	if(m_pNewDuelWindow->Create(m_pNewUIMng, GetWindowsX - 131, 359) == false)
	{
		return false;
	}
	...
	if(m_pNewSiegeWarfare->Create(m_pNewUIMng, GetWindowsX - 154, 234) == false)
		return false;

	m_pNewItemEnduranceInfo = new CNewUIItemEnduranceInfo;
	if( m_pNewItemEnduranceInfo->Create( m_pNewUIMng, 2, 2 ) == false )
	{
		return false;
	}
	...
	if(m_pNewCursedTempleEnterWindow->Create(m_pNewUIMng, (int)setPosCenterX(230), 80) == false)
	{
		return false;
	}
	...
	if(m_pNewCursedTempleResultWindow->Create(m_pNewUIMng, (int)setPosCenterX(230), 120) == false)
	{
		return false;
	}
	...
	if(m_pNewMiniMap->Create(m_pNewUIMng, GetCenterPosition(0), 0) == false)
		return false;

	m_pNewGoldBowman = new CNewUIGoldBowmanWindow;
	if( m_pNewGoldBowman->Create(m_pNewUIMng, GWidescreen.WidescreenPosX1, 0) == false )
		return false;

	m_pNewGoldBowmanLena = new CNewUIGoldBowmanLena;
	if( m_pNewGoldBowmanLena->Create(m_pNewUIMng, GWidescreen.WidescreenPosX1, 0) == false )
		return false;

	m_pNewLuckyCoinRegistration = new CNewUIRegistrationLuckyCoin;
	if( m_pNewLuckyCoinRegistration->Create(m_pNewUIMng, GWidescreen.WidescreenPosX2, 0) == false )
		return false;	

	m_pNewExchangeLuckyCoinWindow = new CNewUIExchangeLuckyCoin;
	if( m_pNewExchangeLuckyCoinWindow->Create(m_pNewUIMng, GWidescreen.WidescreenPosX2, 0) == false )
		return false;

	m_pNewDuelWatchWindow = new CNewUIDuelWatchWindow;
	if( m_pNewDuelWatchWindow->Create(m_pNewUIMng, GWidescreen.WidescreenPosX1, 0) == false )
		return false;
	...
	if(m_pNewInGameShop->Create(m_pNewUIMng, setPosMidRight(0), 0) == false)
		return false;
#endif //PBG_ADD_INGAMESHOP_UI_MAINFRAME

	m_pNewDoppelGangerWindow = new CNewUIDoppelGangerWindow;
	if( m_pNewDoppelGangerWindow->Create(m_pNewUIMng, m_pNewUI3DRenderMng, GWidescreen.WidescreenPosX1, 0) == false )
		return false;

	m_pNewDoppelGangerFrame = new CNewUIDoppelGangerFrame;
	if( m_pNewDoppelGangerFrame->Create(m_pNewUIMng, (GetWindowsX - 227), 480-51-87) == false )
		return false;

	m_pNewNPCDialogue = new CNewUINPCDialogue;
	if (m_pNewNPCDialogue->Create(m_pNewUIMng, GWidescreen.WidescreenPosX1, 0) == false)
		return false;

	m_pNewQuestProgress = new CNewUIQuestProgress;
	if (m_pNewQuestProgress->Create(m_pNewUIMng, GWidescreen.WidescreenPosX1, 0) == false)
		return false;

	m_pNewQuestProgressByEtc = new CNewUIQuestProgressByEtc;
	if (m_pNewQuestProgressByEtc->Create(m_pNewUIMng, GWidescreen.WidescreenPosX1, 0) == false)
		return false;

	m_pNewEmpireGuardianNPC = new CNewUIEmpireGuardianNPC;
	if( m_pNewEmpireGuardianNPC->Create(m_pNewUIMng, m_pNewUI3DRenderMng, GWidescreen.WidescreenPosX1, 0) == false )
		return false;

	m_pNewEmpireGuardianTimer = new CNewUIEmpireGuardianTimer;
	if( m_pNewEmpireGuardianTimer->Create(m_pNewUIMng, GetWindowsX - 127, 342) == false )
		return false;
...
#ifdef NEW_MUHELPER_ON
	SAFE_DELETE(m_GFxMacroMain);
	SAFE_DELETE(m_GFxMacroSubMain);
	SAFE_DELETE(m_GFxMacroGaugeBar);
#endif
...
void SEASON3B::CNewUISystem::Show(DWORD dwKey)
{
	...
	if(m_pNewUIMng) 
	{
		...
		else if(dwKey == SEASON3B::INTERFACE_INVENTORY)
		{
			...
			{
				g_pMyInventory->SetPos(GWidescreen.WidescreenPosX2, 0);
			}
		...
		else if(dwKey == SEASON3B::INTERFACE_CHARACTER)
		{
			...
			{
				g_pMyQuestInfoWindow->SetPos(GWidescreen.WidescreenPosX2, 0);
			}
		...
		else if (dwKey == SEASON3B::INTERFACE_CHAOSGENESIS)
		{
			HideAllGroupA();
			g_pChaosGenesis->OpeningProcess();
			m_pNewUIMng->ShowInterface(SEASON3B::INTERFACE_INVENTORY);
			g_pMainFrame->SetBtnState(MAINFRAME_BTN_MYINVEN, true);
		}
...
#ifdef NEW_MUHELPER_ON
		else if (dwKey == SEASON3B::INTERFACE_MACRO_MAIN)
		{
			HideAllGroupA();
			m_GFxMacroMain->OpeningProcess();
		}
#endif
		else if(dwKey == SEASON3B::INTERFACE_HERO_POSITION_INFO)
		{
#ifndef NEW_MUHELPER_ON
		...
		else if(dwKey == SEASON3B::INTERFACE_QUEST_PROGRESS_ETC)
		{
			...
			{
				Hide(SEASON3B::INTERFACE_MYQUEST);
				g_pQuestProgressByEtc->SetPos(GWidescreen.WidescreenPosX1, 0);
			}
			if (IsVisible(SEASON3B::INTERFACE_CHARACTER))
				g_pQuestProgressByEtc->SetPos(GWidescreen.WidescreenPosX2, 0);
		...
		else if(dwKey == SEASON3B::INTERFACE_GENSRANKING)
		{
			...
			g_pNewUIGensRanking->SetPos(GWidescreen.WidescreenPosX1, 0);
		}
...
void SEASON3B::CNewUISystem::Hide(DWORD dwKey)
{
	if(m_pNewUIMng) 
	{
		...
		else if(dwKey == SEASON3B::INTERFACE_CHARACTER)
		{
			...
			if(IsVisible(SEASON3B::INTERFACE_MYQUEST))
			{
				g_pMyQuestInfoWindow->SetPos(GWidescreen.WidescreenPosX1, 0);
			}
			if(IsVisible((SEASON3B::INTERFACE_INVENTORY)))
			{
				g_pMyInventory->SetPos(GWidescreen.WidescreenPosX1, 0);
			}
			if(IsVisible((SEASON3B::INTERFACE_PET)))
			{
				Hide(SEASON3B::INTERFACE_PET);
		...
		else if(dwKey == SEASON3B::INTERFACE_INVENTORY)
		{
			...
			if (IsVisible(SEASON3B::INTERFACE_CHAOSGENESIS))
			{
				if (g_pChaosGenesis->ClosingProcess() == false)
					return;
				m_pNewUIMng->ShowInterface(SEASON3B::INTERFACE_CHAOSGENESIS, false);
			}
#ifdef LEM_ADD_LUCKYITEM
...
void SEASON3B::CNewUISystem::HideAllGroupA()
{
	...
	{ 
		...
		SEASON3B::INTERFACE_CHAOSGENESIS,
...
bool SEASON3B::CNewUISystem::Update()
{
	...
	g_pChaosGenesis->Scrolling(MouseWheel);
...
{
	if(IsVisible(SEASON3B::INTERFACE_MIXINVENTORY) || IsVisible(SEASON3B::INTERFACE_CHAOSGENESIS)
...
{
	if(IsVisible(SEASON3B::INTERFACE_MIXINVENTORY)
		|| IsVisible(SEASON3B::INTERFACE_CHAOSGENESIS)
		|| IsVisible(SEASON3B::INTERFACE_KANTURU2ND_ENTERNPC)
...
CChaosGenesis* SEASON3B::CNewUISystem::GetUI_ChaosGenesis() const
{
	return m_pChaosGenesis;
}

CStackInterface* SEASON3B::CNewUISystem::GetUI_StackInterface() const
{
	return m_pStackInterface;
}
...
CGFxMacroMain* SEASON3B::CNewUISystem::GetGFxMacroMain() const
{
	return m_GFxMacroMain;
}

CGFxMacroGaugeBar* SEASON3B::CNewUISystem::GetGFxMacroGaugeBar() const
{
	return m_GFxMacroGaugeBar;
}

CGFxMacroSubMain* SEASON3B::CNewUISystem::GetGFxMacroSubMain() const
{
	return m_GFxMacroSubMain;
}
#endif
