...
bool SEASON3B::CNewUIMasterLevel::Create(CNewUIManager* pNewUIMng, int x, int y)
{
	...
	m_BtnExit.ChangeButtonInfo(m_Pos.x + 591 + GetCenterX(640), m_Pos.y + 390, 36, 29);
...
//	float ny = float(429.f * (480.f/(float)WindowHeight));
	...
	m_Pos.x = GetCenterX(640) + (640 - m_Width.x) / 2;
	...
	for (int Skill = 0; Skill < 4; Skill++)
	{
		...
		for (int sy = StartY; sy < (Wid + StartY); sy++)
		{
			...
			for (int sx = 0; sx < m_MasterLevel[GetJob + Skill].Width; sx++)
			{
				...
				RenderImage(IMAGE_MASTER_INTERFACE + 1, Loc[Skill] + ((SKILL_ICON_WIDTH + 19) * sx) + PosX, 70 + ((SKILL_ICON_HEIGHT + 14) * nsy), 35, 34, 0.f, 0.f, nx / 64.f, ny / 64.f, Color);
				RenderImage(IMAGE_MASTER_SKILL_ICON, Loc[Skill] + 5 + ((SKILL_ICON_WIDTH + 19) * sx) + PosX, 75 + ((SKILL_ICON_HEIGHT + 14) * nsy), (float)15, (float)21, (20.f / 256.f) * (Skill_Icon % 12), ((28.f / 256.f) * ((Skill_Icon / 12) + 4)), 20.f / 256.f, 27.5f / 256.f, Color);
				if (Skill_Num > 0)
				{
					...
					g_pRenderText->RenderText(Loc[Skill] + 31 + ((SKILL_ICON_WIDTH + 19) * sx) + PosX, 95 + ((SKILL_ICON_HEIGHT + 14) * nsy), lpszStr, 0, 0, RT3_WRITE_CENTER);
				...
				if (Dir == 1)
					RenderImage(IMAGE_MASTER_INTERFACE + 2, Loc[Skill] + ((SKILL_ICON_WIDTH + 19) * sx) + 9 + PosX, 70 + ((SKILL_ICON_HEIGHT + 14) * nsy) - 12, 8, 12, 0.f, 18.f / 32.f, 14.8f / 16.f, -(18.f / 32.f));
				if (Dir == 2)
					RenderImage(IMAGE_MASTER_INTERFACE + 3, Loc[Skill] + ((SKILL_ICON_WIDTH + 19) * sx) + 27 + PosX, 70 + ((SKILL_ICON_HEIGHT + 14) * nsy) + 11, 12, 8, 0.f, 0.f, 18.f / 32.f, 14.8f / 16.f);
				if (Dir == 3)
					RenderImage(IMAGE_MASTER_INTERFACE + 2, Loc[Skill] + ((SKILL_ICON_WIDTH + 19) * sx) + 9 + PosX, 70 + ((SKILL_ICON_HEIGHT + 14) * nsy) + 31, 8, 12, 0.f, 0.f, 14.8f / 16.f, 18.f / 32.f);
				if (Dir == 4)
					RenderImage(IMAGE_MASTER_INTERFACE + 3, Loc[Skill] + ((SKILL_ICON_WIDTH + 19) * sx) - 12 + PosX, 70 + ((SKILL_ICON_HEIGHT + 14) * nsy) + 11, 12, 8, 18.f / 32.f, 0.f, -(18.f / 32.f), 14.8f / 16.f);
		...
		for (int sy = StartY; sy < (Wid + StartY); sy++)
		{
			...
			for (int sx = 0; sx < m_MasterLevel[GetJob + Skill].Width; sx++)
			{
				...
				if (!(SEASON3B::IsPress(VK_LBUTTON)))
				{
					...
					if (CheckMouseIn(Loc[Skill] + 5 + ((SKILL_ICON_WIDTH + 19) * sx) + PosX, 75 + ((SKILL_ICON_HEIGHT + 14) * nsy), 15, 21))
					{
						...
						RenderTipTextList(Loc[Skill] + 5 + ((SKILL_ICON_WIDTH + 19) * sx) + PosX, 75 + ((SKILL_ICON_HEIGHT + 14) * nsy), ib, 0, RT3_SORT_CENTER, FALSE);
					}
				}
			}
		}
	...
	for (int Skill = 0; Skill < 4; Skill++)
	{
		...
		for (int sy = StartY; sy < (Wid + StartY); sy++)
		{
			...
			for (int sx = 0; sx < m_MasterLevel[GetJob + Skill].Width; sx++)
			{
				...
				if (SEASON3B::IsPress(VK_LBUTTON) && CheckMouseIn(Loc[Skill] + 5 + ((SKILL_ICON_WIDTH + 19) * sx) + PosX, 75 + ((SKILL_ICON_HEIGHT + 14) * nsy), 15, 21))
				{
					...
					if (Skill_Num > 0)
					{
						...
						if (Get_Skill == true)
						{
							...
							if ((Point_Before <= Before_Line) && (Point_R >= Over_Line) && Get_Magic <= 5)
							{
								int Pot = Get_Magic;
								if (Pot < 0)
									Pot = 0;
