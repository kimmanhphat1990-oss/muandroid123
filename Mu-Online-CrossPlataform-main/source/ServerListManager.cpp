...
{
	m_mapServerListScript.clear();

#if defined(__ANDROID__)
	const std::string server_list_path = platform::ResolveGameAssetPath("Data\\Local\\ServerList.bmd");
	FILE* fp = ::fopen(server_list_path.c_str(), "rb");
#else
...
void CServerListManager::InsertServer(CServerGroup* pServerGroup, int iConnectIndex, int iServerPercent)
{
	...
	pServerInfo->m_iIndex = (iConnectIndex) + 1;
	pServerInfo->m_iConnectIndex = iConnectIndex;
	pServerInfo->m_iPercent = iServerPercent;
	pServerInfo->m_byNonPvP = pServerGroup->m_abyNonPvpServer[((iConnectIndex % 20) + 1 )-1];
...
#if !defined(__ANDROID__)
	auto serverInfo = gCustomServerName.GetServer(pServerInfo->m_iIndex);

	if (serverInfo)
	{
		if (pServerInfo->m_iIndex == serverInfo->ServerID)
		{
			std::memcpy(pServerInfo->m_bName, serverInfo->ServerName, sizeof(serverInfo->ServerName));
		}
	}
	else
#endif
