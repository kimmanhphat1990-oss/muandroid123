...
namespace
{
	void SetSideHairVertex(platform::Vertex3D* vertex, float x, float y, float z, float u, float v)
	{
		vertex->x = x;
		vertex->y = y;
		vertex->z = z;
...
void CSideHair::RenderLine( vec3_t v1, vec3_t v2, vec3_t c1, vec3_t c2)
{
	...
	SetSideHairVertex(&quad[0], p1[0]-vOrtho[0], p1[1]-vOrtho[1], p1[2]-vOrtho[2], 0.f, 0.f+fTextureMove+fTextureV);
	SetSideHairVertex(&quad[1], p2[0]-vOrtho[0], p2[1]-vOrtho[1], p2[2]-vOrtho[2], 0.f, 1.f-fTextureMove+fTextureV);
	SetSideHairVertex(&quad[2], p2[0]+vOrtho[0], p2[1]+vOrtho[1], p2[2]+vOrtho[2], 1.f, 1.f-fTextureMove+fTextureV);
	SetSideHairVertex(&quad[3], p1[0]+vOrtho[0], p1[1]+vOrtho[1], p1[2]+vOrtho[2], 1.f, 0.f+fTextureMove+fTextureV);
