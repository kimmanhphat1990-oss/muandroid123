...
namespace
{
	void SetMagicVertex3D(platform::Vertex3D* vertex, const vec3_t position, float u, float v, const vec3_t light)
	{
		vertex->x = position[0];
		vertex->y = position[1];
		vertex->z = position[2];
	...
	void SetMagicVertex2D(platform::QuadVertex2D* vertex, float x, float y, float u, float v, const vec3_t light)
	{
		vertex->x = x;
		vertex->y = y;
		vertex->u = u;
...
void RenderCircle(int Type, vec3_t ObjectPosition, float ScaleBottom, float ScaleTop, float Height, float Rotation, float LightTop, float TextureV)
{
	...
	for (float x = 0.f; x < Num; x += 1.f)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			SetMagicVertex3D(&quad[i], Position[i], UV[i][0], UV[i][1] + TextureV, Light[i]);
		}
...
void RenderCircle2D(int Type, vec3_t ScreenPosition, float ScaleBottom, float ScaleTop, float Height, float Rotation, float TextureV, float TextureVScale)
{
	...
	for (float x = 0.f; x < Num; x += 1.f)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			...
			SetMagicVertex2D(&quad[i], Position[i][0], Position[i][1], UV[i][0], UV[i][1] + TextureV, Light[i]);
		}
		platform::GetRenderBackend().DrawQuad2D(quad, true);
	}
}
