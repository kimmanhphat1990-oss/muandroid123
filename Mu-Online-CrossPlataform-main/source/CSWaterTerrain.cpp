...
namespace
{
    void SetWaterVertex(platform::Vertex3D& vertex, const vec3_t position, float u, float v, float r, float g, float b, float a)
    {
        vertex.x = position[0];
        vertex.y = position[1];
        vertex.z = position[2];
...
void    CSWaterTerrain::Render ( void )
{
    ...
    std::vector<platform::Vertex3D> base_vertices;
    std::vector<platform::Vertex3D> reflection_vertices;
    base_vertices.reserve(m_iTriangleListNum);
    reflection_vertices.reserve(m_iTriangleListNum);
	for ( j=0; j<m_iTriangleListNum; j++ )
	{
        offset = m_iTriangleList[j];
        platform::Vertex3D base_vertex;
        SetWaterVertex(base_vertex, m_Vertices[offset], g_chrome[offset][1], g_chrome[offset][0], 0.2f, 0.5f, 0.65f, 1.0f);
        base_vertices.push_back(base_vertex);

        alpha = 1.f-DotProduct ( m_Normals[offset], m_vLightVector );
        platform::Vertex3D reflection_vertex;
        SetWaterVertex(reflection_vertex, m_Vertices[offset], g_chrome[offset][1], g_chrome[offset][0], alpha, alpha*2.5f, alpha*3.f, 1.0f);
        reflection_vertices.push_back(reflection_vertex);
	}
    if (!base_vertices.empty())
    {
        platform::GetRenderBackend().DrawTriangleList3D(&base_vertices[0], static_cast<int>(base_vertices.size()), true, true);
    }
    EnableAlphaBlend ();
	BindTexture ( BITMAP_MAPTILE+1 );
    if (!reflection_vertices.empty())
    {
        platform::GetRenderBackend().DrawTriangleList3D(&reflection_vertices[0], static_cast<int>(reflection_vertices.size()), true, true);
    }
}
...
void    CSWaterTerrain::RenderWaterBitmapTile(float xf,float yf,float lodf,int lodi,vec3_t c[4],bool LightEnable,float Alpha,float Height)
{
	...
	for(int i=0;i<4;i++)
	{
        ...
        SetWaterVertex(vertices[i], TerrainVertex[i], c[i][0], c[i][1], r, g, b, Alpha);
	}
    platform::GetRenderBackend().DrawQuad3D(vertices, true, LightEnable);
}
