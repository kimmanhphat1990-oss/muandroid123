	...
	for (y = 0; y < m_nRow; y++)
	{
		for (x = 0; x < m_nColumn; x++)
		{
			...
			if (m_pdwItemCheckBox[iCurSquareIndex] > 1)
			{
				...
				{
					BeginBitmap();
					glColor4f(1.0, 1.0, 1.0, 0.3); //Bota isso antes da imagem
					RenderBitmap(BITMAP_INVENTORY + 18, m_Pos.x + (x * INVENTORY_SQUARE_WIDTH), m_Pos.y + (y * INVENTORY_SQUARE_HEIGHT), 21, 21, 0, 0, 1.0, 1.0, 1, 1, 0.0);
					...
					case 5:
						...
						break;
					default:
						RenderBitmap(BITMAP_INVENTORY + 17, m_Pos.x + (x * INVENTORY_SQUARE_WIDTH), m_Pos.y + (y * INVENTORY_SQUARE_HEIGHT), 21, 21, 0, 0, 1.0, 1.0, 1, 1, 0.0);
						break;
					}
							...
							for(int x=0; x<nItemColumn; x++)
							{
								...
								if(m_pdwItemCheckBox[iCurSquareIndex] > 1)
								{
									...
									if(pTargetItem)
									{
										...
										if (pPickItem->Type == iType && gJewelStack.CheckIsStack(pPickItem->Type))
										{
											bSuccess = true;
										}
										...
										if (bSuccess == false && m_pOwner == g_pMyInventory)
										{
											...
											{
												bSuccess = gCustomJewel.CheckCustomJewelApplyItem(pPickItem->Type, pTargetItem->Type);
											}
										}
...
void SEASON3B::CNewUIInventoryCtrl::RenderNumberOfItem()
{
	...
	for(; li != m_vecItem.end(); li++)
	{
		...
		else if (gJewelStack.CheckIsStack(pItem->Type) && pItem->Durability > 1)
		{
			glColor3f(1.f, 0.9f, 0.7f);
			SEASON3B::RenderNumber(x + width - 6, y + 1, pItem->Durability);
		}
...
void SEASON3B::CNewUIInventoryCtrl::BackupPickedItem()
{
	if(ms_pPickedItem && EquipmentItem == false)
	{
		...
		else if(pItemObj->ex_src_type == ITEM_EX_SRC_EQUIPMENT)
		{
			...
			if (pItemObj->lineal_pos >= 236 && pItemObj->lineal_pos <= 239)
			{
				gVisualInventory.EffectPet(pItemObj->lineal_pos, pEquipmentItemSlot);
			}
			if (pItemObj->lineal_pos >= 240 && pItemObj->lineal_pos <= 249)
			{
				gVisualInventory.EffectHelperVisual(pEquipmentItemSlot);
			}
			else
			{
			...
			if (gDarkSpirit.checkIsDarkSpirit(pEquipmentItemSlot->Type)) 
			{
				auto hero = Hero;
				auto backupWeapon = hero->Weapon[1].Type;
				hero->Weapon[1].Type = pEquipmentItemSlot->Type + MODEL_ITEM;
...
bool SEASON3B::CNewUIInventoryCtrl::IsOverlayItem(ITEM* pSourceItem, ITEM* pTargetItem)
{
	...
	if(iSrcType == iTarType)
	{
		...
		else if (iSrcType == iTarType && gJewelStack.CheckIsStack(iSrcType)) 
		{
			return true;
		}
	}
