...
void SEASON3B::CNewUICommandWindow::InitButtons( )
{
	...
	for(int i=COMMAND_TRADE ; i<COMMAND_END ; i++)
	{
		...
		m_BtnCommand[i].ChangeButtonInfo( m_Pos.x+(COMMAND_WINDOW_WIDTH/2-108/2), (m_Pos.y+59)+(i*(29+COMMAND_BTN_INTERVAL_SIZE)), 120, 29 );
	}
...
bool SEASON3B::CNewUICommandWindow::BtnProcess()
{
	...
	for(int i=COMMAND_TRADE ; i<COMMAND_END ; i++)
	{
		...
		{
#ifdef PC_POINT
			if (i == COMMAND_PCBANG) { return CommandPCBangPoint(); }
#endif
...
bool SEASON3B::CNewUICommandWindow::CommandTrade(CHARACTER* pSelectedCha)
{
	...
	SendRequestTrade ( pSelectedCha->Key , 0);

	return true;
}

bool SEASON3B::CNewUICommandWindow::CommandTradeX(CHARACTER* pSelectedCha)
{
	if (pSelectedCha == NULL)
		return false;
	...
	SendRequestTrade(pSelectedCha->Key, 1);
...
bool SEASON3B::CNewUICommandWindow::CommandGuild(CHARACTER* pSelectedChar)
{
	...
	if( (pSelectedChar->GuildMarkIndex < 0) || (pSelectedChar->GuildStatus != G_MASTER && pSelectedChar->GuildStatus != G_SUB_MASTER) )
	{
...
bool SEASON3B::CNewUICommandWindow::CommandGuildUnion(CHARACTER* pSelectedCha)
{
	...
	if( pSelectedCha->GuildStatus != G_MASTER && pSelectedCha->GuildStatus != G_SUB_MASTER)
	{
		g_pChatListBox->AddText("", GlobalText[507], SEASON3B::TYPE_SYSTEM_MESSAGE);
		return false;
	}	
