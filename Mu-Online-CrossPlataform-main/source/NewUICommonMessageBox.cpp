...
void SEASON3B::CNewUIMessageBoxButton::Render()
{
...
#ifdef KJH_ADD_INGAMESHOP_UI_SYSTEM
	if (m_dwTexType == 931324)
	{
		GWidescreen.RenderImages(m_dwTexType, m_x, m_y, m_width, m_height, 0.0, m_EventState * 54.f, 294.5f, 53.5f);
	}
	else if (m_dwTexType == 931325 || m_dwTexType == 31324 || m_dwTexType == 31322) //-- boton cerrar
	{
		GWidescreen.RenderImages(931325, m_x, m_y, 54, 23, 0.0, m_EventState * 54.f, 122.f, 53.5f);
	}
	else if (m_dwTexType == 931327 || m_dwTexType == 931326) //-- boton cerrar
	{
		GWidescreen.RenderImages(m_dwTexType, m_x, m_y, 54, 23, 0.0, m_EventState * 54.f, 122.f, 53.5f);
	}
	else if (m_dwTexType == 941101) //-- boton cerrar
	{
		GWidescreen.RenderImages(m_dwTexType, m_x, m_y, 120, 27, 0.0, m_EventState * 54.f, 200.f, 53.5f);
	}
	else if (m_dwTexType == 941102) //-- boton cerrar
	{
		GWidescreen.RenderImages(m_dwTexType, m_x, m_y, 88, 27, 0.0, m_EventState * 54.f, 122.f, 53.5f);
	}
	else if (m_dwTexType == 31320)
	{
		GWidescreen.RenderImages(931327, m_x, m_y, 54, 23, 0.0, m_EventState * 54.f, 122.f, 53.5f);
	}
	else if (m_dwTexType == 31321) //-- boton cerrar
	{
		GWidescreen.RenderImages(931326, m_x, m_y, 54, 23, 0.0, m_EventState * 54.f, 122.f, 53.5f);
	}
	else
	{
		BITMAP_t* pImage = &Bitmaps[m_dwTexType];
		RenderBitmap(m_dwTexType, m_x, m_y, m_width, m_height, (0.5f / (float)pImage->Width), ((m_EventState * m_fButtonHeight + 0.5f) / (float)pImage->Height), (m_fButtonWidth - 0.5f) / (float)pImage->Width - (0.5f / (float)pImage->Width), (m_fButtonHeight - 0.5f) / (float)pImage->Height - (0.5f / (float)pImage->Height));
	}
#else //KJH_ADD_INGAMESHOP_UI_SYSTEM
	float fv = 0.f;
	...
	if(m_strText.size() > 0)
	{
		...
		Fontsize.cx = Fontsize.cx / ((float)WindowWidth / GWidescreen.JCWinWidth);
...
bool SEASON3B::CNewUICommonMessageBox::Create(DWORD dwType, float fPriority)
{
	...
	x = (GWidescreen.JCWinWidth / 2) - (MSGBOX_WIDTH / 2);
	y = 100;
	width = MSGBOX_WIDTH;
...
void SEASON3B::CNewUICommonMessageBox::RenderTexts()
{
	...
	for(; vi != m_MsgDataList.end(); vi++)
	{
		...
		g_pRenderText->RenderText((int)x, (int)y - 10, (*vi)->strMsg.c_str());
...
bool SEASON3B::CMapEnterWerwolfMsgBoxLayout::SetLayout()
{
	...
	pMsgBox->SetPos((GetWindowsX / 2) - (MSGBOX_WIDTH / 2), 50);
...
bool SEASON3B::CBuyVaultMsgBoxLayout::SetLayout()
{
	CNewUICommonMessageBox* pMsgBox = GetMsgBox();
	...
	pMsgBox->AddCallbackFunc(CBuyVaultMsgBoxLayout::OkBtnDown, MSGBOX_EVENT_USER_COMMON_OK);
	pMsgBox->AddCallbackFunc(CBuyVaultMsgBoxLayout::CancelBtnDown, MSGBOX_EVENT_USER_COMMON_CANCEL);
	pMsgBox->AddCallbackFunc(CBuyVaultMsgBoxLayout::CancelBtnDown, MSGBOX_EVENT_PRESSKEY_ESC);
	
	return true;
}

CALLBACK_RESULT SEASON3B::CBuyVaultMsgBoxLayout::OkBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	SendRequestVaultBuy();
	PlayBuffer(SOUND_CLICK01);
	g_MessageBox->SendEvent(pOwner, MSGBOX_EVENT_DESTROY);
...
CALLBACK_RESULT SEASON3B::CBuyVaultMsgBoxLayout::CancelBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	PlayBuffer(SOUND_CLICK01);
	g_MessageBox->SendEvent(pOwner, MSGBOX_EVENT_DESTROY);
