...
void ItemConvert(ITEM *ip,BYTE Attribute1,BYTE Attribute2, BYTE Attribute3 )
{
	...
	else if (p->RequireLevel && ((ip->Type >= ITEM_WING && ip->Type <= ITEM_WING + 7) || (ip->Type >= ITEM_WING + 41 && ip->Type <= ITEM_WING + 42) || ip->Type >= ITEM_HELPER))
	...
	if(Option3)
	{
        ...
        else
        {
			...
			else if (gCustomWing.CheckCustomWingByItem(ip->Type))
			{
				if ((excelWing >> 4) & 0x01)
				{
					ip->SpecialValue[ip->SpecialNum] = gCustomWing.GetCustomWingOptionValue(ip->Type, 1);
					ip->Special[ip->SpecialNum] = gCustomWing.GetCustomWingOptionIndex(ip->Type, 1);
					ip->SpecialNum++;
				}
				else if ((excelWing >> 5) & 0x01)
				{
					ip->SpecialValue[ip->SpecialNum] = gCustomWing.GetCustomWingOptionValue(ip->Type, 2);
					ip->Special[ip->SpecialNum] = gCustomWing.GetCustomWingOptionIndex(ip->Type, 2);
					ip->SpecialNum++;
				}
				else
				{
					ip->SpecialValue[ip->SpecialNum] = gCustomWing.GetCustomWingOptionValue(ip->Type, 0);
					ip->Special[ip->SpecialNum] = gCustomWing.GetCustomWingOptionIndex(ip->Type, 0);
					ip->SpecialNum++;
				}
			}
        }
	...
	else if(ip->Type>=ITEM_WING+36 && ip->Type<=ITEM_WING+43 || gCustomWing.CheckCustomWingByItem(ip->Type))
		ip->Part = EQUIPMENT_WING;
	...
	gCustomItemForce.ItemConverts(ip, Attribute1, Attribute2, Attribute3);
}
...
int ItemValue(ITEM *ip,int goldType)
{
	...
	else if(ip->Type==ITEM_POTION+13 || gCustomJewel.CheckCustomJewelByItem(ip->Type))
	{
		if (ip->Type == ITEM_POTION + 13)
		{
			Gold = 9000000;
		}
		if (gCustomJewel.CheckCustomJewelByItem(ip->Type))
		{
			Gold = gCustomJewel.GetCustomJewelSalePrice(ip->Type);
		}
	}
	...
	if (CPcPoints::Instance().IsPCRoomPointShopMode())
	{
		if (ip->Type >= ITEM_HELPER && ip->Type <= ITEM_ETC + 511)
		{
			Gold = 100;	
		}
	}
