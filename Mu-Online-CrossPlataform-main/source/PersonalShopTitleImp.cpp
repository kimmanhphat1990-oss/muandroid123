...
void CPersonalShopTitleImp::UpdatePosition()
{
	...
	if (m_Resolution > 3) { Width = GetWindowsX, Height = GetWindowsY; } else { Width = 640; Height = 480; }
...
void CPersonalShopTitleImp::CalculateBooleanPos(IN CHARACTER* pPlayer, IN const SIZE& size, OUT POINT& pos)
{
	...
	//. pos : real position
	pos.x = (ptFloating.x * (int)WindowWidth / GetWindowsX) - size.cx / 2;
	pos.y = (ptFloating.y * (int)WindowHeight / GetWindowsY) - (12 * 3 * (int)WindowHeight / GetWindowsY);
}
