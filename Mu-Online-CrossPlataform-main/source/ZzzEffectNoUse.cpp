...
namespace
{
    void SetEffectNoUseVertex3D(platform::Vertex3D* vertex, const vec3_t position, float u, float v, float r, float g, float b, float a)
    {
        vertex->x = position[0];
        vertex->y = position[1];
        vertex->z = position[2];
    ...
    void SetEffectNoUseTriangleVertex(platform::Vertex3D* vertex, float x, float y, float z, float u, float v, float r, float g, float b, float a)
    {
        vertex->x = x;
        vertex->y = y;
        vertex->z = z;
...
void RenderPlane(int Texture, vec3_t Position, vec3_t Light, float Scale, float Angle)
{
    BindTexture(Texture);
    EnableAlphaBlend();

    platform::GetRenderBackend().PushModelView();
    platform::GetRenderBackend().TranslateModelView(Position[0], Position[1], Position[2]);
    platform::GetRenderBackend().RotateModelView(Angle, 0.f, 0.f, 1.f);
    ...
    SetEffectNoUseVertex3D(&quad[0], BoundingVertices[0], 0.f, 1.f, Light[0], Light[1], Light[2], 1.f);
    SetEffectNoUseVertex3D(&quad[1], BoundingVertices[2], 1.f, 1.f, Light[0], Light[1], Light[2], 1.f);
    SetEffectNoUseVertex3D(&quad[2], BoundingVertices[3], 1.f, 0.f, Light[0], Light[1], Light[2], 1.f);
    SetEffectNoUseVertex3D(&quad[3], BoundingVertices[1], 0.f, 0.f, Light[0], Light[1], Light[2], 1.f);
...
void RenderPlanes()
{
    for (int i = 0; i < MAX_PLANES; i++)
    {
        ...
        if (o->Live)
        {
            RenderPlane(o->Type, o->Position, o->Light, o->Scale, o->Angle[2]);
        }
    }
}
    ...
    for (float y = 0.f; y < Height; y += 1.f)
    {
        for (float x = 0.f; x < Width; x += 1.f)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                SetEffectNoUseVertex3D(&quad[i], Position[i], UV[i][0], UV[i][1] + TextureV, Light[0], Light[1], Light[2], 1.f);
            }
            platform::GetRenderBackend().DrawQuad3D(quad, true, true);
        }
    }
...
void MagicBitmap(int Texture)
{
    ...
    platform::LegacyCompatibleTexImage2D(GL_TEXTURE_2D, 0, b->Components, (int)b->Width, (int)b->Height, 0, GL_RGB, GL_UNSIGNED_BYTE, b->Buffer);
}
...
void RenderMagicBox(int Texture, vec3_t Position, float Angle)
{
    ...
    static const int kFaceIndices[5][4] =
    {
        {7, 6, 4, 5},
        {0, 2, 3, 1},
        {7, 3, 2, 6},
    ...
    static const float kFaceUV[4][2] =
    {
        {1.f, 1.f},
        {1.f, 0.f},
        {0.f, 0.f},
    ...
    for (int face = 0; face < 5; ++face)
    {
        for (int vertex_index = 0; vertex_index < 4; ++vertex_index)
        {
            ...
            SetEffectNoUseVertex3D(&quad[vertex_index], BoundingVertices[source_index], kFaceUV[vertex_index][0], kFaceUV[vertex_index][1], Color, Color, Color, 1.f);
        }
        platform::GetRenderBackend().DrawQuad3D(quad, true, true);
    }
...
void RenderFog(vec3_t Position, int Flag, float Height, float u, float v)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        SetEffectNoUseVertex3D(&quad[i], p[i], UV[i][0], UV[i][1], 1.f, 1.f, 1.f, Alpha[i]);
    }
    platform::GetRenderBackend().DrawQuad3D(quad, true, true);
}
...
void MappingEffect(int SrcIndex, int DstIndex, int Flag)
{
    ...
    platform::LegacyCompatibleTexImage2D(GL_TEXTURE_2D, 0, src->Components, (int)src->Width, (int)src->Height, 0, GL_RGB, GL_UNSIGNED_BYTE, dst->Buffer);
}
...
void RenderSky3()
{
    ...
    for (float x = 0.f; x < Num; x += 1.f)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            SetEffectNoUseVertex3D(&quad[i], Position[i], UV[i][0], UV[i][1], Light[i][0], Light[i][1], Light[i][2], 1.f);
            quad[i].z += 100.f;
        }
        platform::GetRenderBackend().DrawQuad3D(quad, true, true);
    }
...
void RenderBackLight(OBJECT* o, vec3_t Position, vec3_t Light1, vec3_t Light2)
{
    ...
    platform::Vertex3D front_vertices[5 * 5 * 6];
    for (int i = 0; i < iVertex; i++)
    {
        offset = g_backLightIndex[i];
        SetEffectNoUseTriangleVertex(
            &front_vertices[i],
            position[0] + g_vertX[offset],
            position[1] + g_vertY[offset],
            position[2] + g_vertZ[offset],
            g_U[offset],
            g_V[offset],
            Light1[0] * light,
            Light1[1] * light,
    ...
    platform::GetRenderBackend().DrawTriangleList3D(front_vertices, iVertex, true, true);

    platform::Vertex3D back_vertices[5 * 5 * 6];
    for (int i = 0; i < iVertex; i++)
    {
        offset = g_backLightIndex[i];
        SetEffectNoUseTriangleVertex(
            &back_vertices[i],
            position[0] + g_vertX2[offset],
            position[1] + g_vertY[offset],
            position[2] + g_vertZ[offset],
            g_U[offset],
            g_V[offset],
    ...
    platform::GetRenderBackend().DrawTriangleList3D(back_vertices, iVertex, true, true);
