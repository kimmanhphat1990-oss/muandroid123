...
{
	(void)bClearWhisperTarget;
	memset(InputText, 0, sizeof(InputText));
}

void RenderInputText(int x, int y, int Index, int Hide)
{
	(void)Hide;
	if (Index < 0 || Index >= 12 || InputText[Index][0] == '\0')
	{
		return;
	}

	g_pRenderText->RenderText(x, y, InputText[Index]);
}
#else
...
void CCharMakeWin::RenderControls()
{
	...
	const char* apszStat[MAX_CLASS][4] =
	{
		"18", "18", "15", "30",
		"28", "20", "25", "10",
		"22", "25", "20", "15",
...
{
	if (nIndex < 0 || nIndex >= MAX_CLASS)
	{
		return NULL;
	}

	return &m_abtnJob[nIndex];
}

const CButton* CCharMakeWin::GetActionButton(int nIndex) const
{
	...
	return &m_aBtn[nIndex];
}

const CSprite* CCharMakeWin::GetBackSprite(int nIndex) const
{
	if (nIndex < 0 || nIndex >= CMW_SPR_MAX)
	{
		return NULL;
	}

	return &m_asprBack[nIndex];
}
